...
- **GitHub description (hint only):** `Turn-based tactical strategy game (WEGO) with simultaneous turns on hexagonal board - Godot 4.4 + FastAPI`
- **Output file (WRITE ONLY THIS):** `/srv/dev/kodexArg/kdx-rag/github-private-repos/syv-godot.md`
...
1. Prefer a **shallow clone** into a temp dir you own, e.g. `/tmp/gh-swarm/syv-godot`:
   ```bash
   rm -rf /tmp/gh-swarm/syv-godot
   mkdir -p /tmp/gh-swarm
   gh repo clone kodexArg/syv-godot /tmp/gh-swarm/syv-godot -- --depth 1 --branch master
   ```
   If branch fails, clone default without `--branch`.
2. Work **only** inside that clone + the template + the single output path.
3. Do **not** modify the clone. Do **not** push. Do **not** change kdx-rag except writing `/srv/dev/kodexArg/kdx-rag/github-private-repos/syv-godot.md`.
...
- One file only at `/srv/dev/kodexArg/kdx-rag/github-private-repos/syv-godot.md`. Filename: `syv-godot.md` (keep exact GitHub casing).

## Done criteria

1. `/srv/dev/kodexArg/kdx-rag/github-private-repos/syv-godot.md` exists and is non-trivial (aim for rich detail; short stubs fail).
2. No secrets in the file.
3. Template section skeleton respected.
4. Print a one-line summary to stdout: `OK syv-godot → /srv/dev/kodexArg/kdx-rag/github-private-repos/syv-godot.md` or `FAIL …`.
