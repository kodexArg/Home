...
- **Repo name:** `alvs-financial-gateway`
- **Visibility flag from GitHub:** `private`  → frontmatter `visibility` must be exactly `public` or `private`
- **Default branch:** `main`
- **GitHub description (hint only):** `ALVS group financial gateway — company financial data from SharePoint to the web through an authenticated AWS tier. Born as a fork of the astro-drf-aws template.`
- **Output file (WRITE ONLY THIS):** `/srv/dev/kodexArg/kdx-rag/github-private-repos/alvs-financial-gateway.md`
...
1. Prefer a **shallow clone** into a temp dir you own, e.g. `/tmp/gh-swarm/alvs-financial-gateway`:
   ```bash
   rm -rf /tmp/gh-swarm/alvs-financial-gateway
   mkdir -p /tmp/gh-swarm
   gh repo clone kodexArg/alvs-financial-gateway /tmp/gh-swarm/alvs-financial-gateway -- --depth 1 --branch main
   ```
   If branch fails, clone default without `--branch`.
2. Work **only** inside that clone + the template + the single output path.
3. Do **not** modify the clone. Do **not** push. Do **not** change kdx-rag except writing `/srv/dev/kodexArg/kdx-rag/github-private-repos/alvs-financial-gateway.md`.
...
- One file only at `/srv/dev/kodexArg/kdx-rag/github-private-repos/alvs-financial-gateway.md`. Filename: `alvs-financial-gateway.md` (keep exact GitHub casing).

## Done criteria

1. `/srv/dev/kodexArg/kdx-rag/github-private-repos/alvs-financial-gateway.md` exists and is non-trivial (aim for rich detail; short stubs fail).
2. No secrets in the file.
3. Template section skeleton respected.
4. Print a one-line summary to stdout: `OK alvs-financial-gateway → /srv/dev/kodexArg/kdx-rag/github-private-repos/alvs-financial-gateway.md` or `FAIL …`.
