...
- **Repo name:** `multimedia-handler-for-rpi`
- **Visibility flag from GitHub:** `private`  → frontmatter `visibility` must be exactly `public` or `private`
- **Default branch:** `main`
- **GitHub description (hint only):** `This project helps organizing videos in order to display them in the Raspberry Pi multimedia network.`
- **Output file (WRITE ONLY THIS):** `/srv/dev/kodexArg/kdx-rag/github-private-repos/multimedia-handler-for-rpi.md`
...
1. Prefer a **shallow clone** into a temp dir you own, e.g. `/tmp/gh-swarm/multimedia-handler-for-rpi`:
   ```bash
   rm -rf /tmp/gh-swarm/multimedia-handler-for-rpi
   mkdir -p /tmp/gh-swarm
   gh repo clone kodexArg/multimedia-handler-for-rpi /tmp/gh-swarm/multimedia-handler-for-rpi -- --depth 1 --branch main
   ```
   If branch fails, clone default without `--branch`.
2. Work **only** inside that clone + the template + the single output path.
3. Do **not** modify the clone. Do **not** push. Do **not** change kdx-rag except writing `/srv/dev/kodexArg/kdx-rag/github-private-repos/multimedia-handler-for-rpi.md`.
...
- One file only at `/srv/dev/kodexArg/kdx-rag/github-private-repos/multimedia-handler-for-rpi.md`. Filename: `multimedia-handler-for-rpi.md` (keep exact GitHub casing).

## Done criteria

1. `/srv/dev/kodexArg/kdx-rag/github-private-repos/multimedia-handler-for-rpi.md` exists and is non-trivial (aim for rich detail; short stubs fail).
2. No secrets in the file.
3. Template section skeleton respected.
4. Print a one-line summary to stdout: `OK multimedia-handler-for-rpi → /srv/dev/kodexArg/kdx-rag/github-private-repos/multimedia-handler-for-rpi.md` or `FAIL …`.
