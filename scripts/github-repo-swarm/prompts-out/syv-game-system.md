...
- **Repo name:** `syv-game-system`
- **Visibility flag from GitHub:** `private`  → frontmatter `visibility` must be exactly `public` or `private`
- **Default branch:** `main`
- **GitHub description (hint only):** `Platform-agnostic rules, mechanics and data models (Obsidian vault) for the simultaneous-turn strategy game Subordinación y Valor.`
- **Output file (WRITE ONLY THIS):** `/srv/dev/kodexArg/kdx-rag/github-private-repos/syv-game-system.md`
...
1. Prefer a **shallow clone** into a temp dir you own, e.g. `/tmp/gh-swarm/syv-game-system`:
   ```bash
   rm -rf /tmp/gh-swarm/syv-game-system
   mkdir -p /tmp/gh-swarm
   gh repo clone kodexArg/syv-game-system /tmp/gh-swarm/syv-game-system -- --depth 1 --branch main
   ```
   If branch fails, clone default without `--branch`.
2. Work **only** inside that clone + the template + the single output path.
3. Do **not** modify the clone. Do **not** push. Do **not** change kdx-rag except writing `/srv/dev/kodexArg/kdx-rag/github-private-repos/syv-game-system.md`.
...
- One file only at `/srv/dev/kodexArg/kdx-rag/github-private-repos/syv-game-system.md`. Filename: `syv-game-system.md` (keep exact GitHub casing).

## Done criteria

1. `/srv/dev/kodexArg/kdx-rag/github-private-repos/syv-game-system.md` exists and is non-trivial (aim for rich detail; short stubs fail).
2. No secrets in the file.
3. Template section skeleton respected.
4. Print a one-line summary to stdout: `OK syv-game-system → /srv/dev/kodexArg/kdx-rag/github-private-repos/syv-game-system.md` or `FAIL …`.
