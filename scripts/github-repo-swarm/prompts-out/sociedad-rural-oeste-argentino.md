...
- **Repo name:** `sociedad-rural-oeste-argentino`
- **Visibility flag from GitHub:** `private`  → frontmatter `visibility` must be exactly `public` or `private`
- **Default branch:** `main`
- **GitHub description (hint only):** `Sitio web institucional + blog de la Sociedad Rural del Oeste Argentino (SROA)`
- **Output file (WRITE ONLY THIS):** `/srv/dev/kodexArg/kdx-rag/github-private-repos/sociedad-rural-oeste-argentino.md`
...
1. Prefer a **shallow clone** into a temp dir you own, e.g. `/tmp/gh-swarm/sociedad-rural-oeste-argentino`:
   ```bash
   rm -rf /tmp/gh-swarm/sociedad-rural-oeste-argentino
   mkdir -p /tmp/gh-swarm
   gh repo clone kodexArg/sociedad-rural-oeste-argentino /tmp/gh-swarm/sociedad-rural-oeste-argentino -- --depth 1 --branch main
   ```
   If branch fails, clone default without `--branch`.
2. Work **only** inside that clone + the template + the single output path.
3. Do **not** modify the clone. Do **not** push. Do **not** change kdx-rag except writing `/srv/dev/kodexArg/kdx-rag/github-private-repos/sociedad-rural-oeste-argentino.md`.
...
- One file only at `/srv/dev/kodexArg/kdx-rag/github-private-repos/sociedad-rural-oeste-argentino.md`. Filename: `sociedad-rural-oeste-argentino.md` (keep exact GitHub casing).

## Done criteria

1. `/srv/dev/kodexArg/kdx-rag/github-private-repos/sociedad-rural-oeste-argentino.md` exists and is non-trivial (aim for rich detail; short stubs fail).
2. No secrets in the file.
3. Template section skeleton respected.
4. Print a one-line summary to stdout: `OK sociedad-rural-oeste-argentino → /srv/dev/kodexArg/kdx-rag/github-private-repos/sociedad-rural-oeste-argentino.md` or `FAIL …`.
