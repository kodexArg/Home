...
- **Repo name:** `DjangoConda`
- **Visibility flag from GitHub:** `private`  → frontmatter `visibility` must be exactly `public` or `private`
- **Default branch:** `master`
- **GitHub description (hint only):** `conda con python37, psycopg2, wsgi y basicos de django`
- **Output file (WRITE ONLY THIS):** `/srv/dev/kodexArg/kdx-rag/github-private-repos/DjangoConda.md`
...
1. Prefer a **shallow clone** into a temp dir you own, e.g. `/tmp/gh-swarm/DjangoConda`:
   ```bash
   rm -rf /tmp/gh-swarm/DjangoConda
   mkdir -p /tmp/gh-swarm
   gh repo clone kodexArg/DjangoConda /tmp/gh-swarm/DjangoConda -- --depth 1 --branch master
   ```
   If branch fails, clone default without `--branch`.
2. Work **only** inside that clone + the template + the single output path.
3. Do **not** modify the clone. Do **not** push. Do **not** change kdx-rag except writing `/srv/dev/kodexArg/kdx-rag/github-private-repos/DjangoConda.md`.
...
- One file only at `/srv/dev/kodexArg/kdx-rag/github-private-repos/DjangoConda.md`. Filename: `DjangoConda.md` (keep exact GitHub casing).

## Done criteria

1. `/srv/dev/kodexArg/kdx-rag/github-private-repos/DjangoConda.md` exists and is non-trivial (aim for rich detail; short stubs fail).
2. No secrets in the file.
3. Template section skeleton respected.
4. Print a one-line summary to stdout: `OK DjangoConda → /srv/dev/kodexArg/kdx-rag/github-private-repos/DjangoConda.md` or `FAIL …`.
