...
- **Repo name:** `django-captive-portal-oauth`
- **Visibility flag from GitHub:** `private`  → frontmatter `visibility` must be exactly `public` or `private`
- **Default branch:** `main`
- **GitHub description (hint only):** `Django Captive Portal with Facebook/Google.`
- **Output file (WRITE ONLY THIS):** `/srv/dev/kodexArg/kdx-rag/github-private-repos/django-captive-portal-oauth.md`
...
1. Prefer a **shallow clone** into a temp dir you own, e.g. `/tmp/gh-swarm/django-captive-portal-oauth`:
   ```bash
   rm -rf /tmp/gh-swarm/django-captive-portal-oauth
   mkdir -p /tmp/gh-swarm
   gh repo clone kodexArg/django-captive-portal-oauth /tmp/gh-swarm/django-captive-portal-oauth -- --depth 1 --branch main
   ```
   If branch fails, clone default without `--branch`.
2. Work **only** inside that clone + the template + the single output path.
3. Do **not** modify the clone. Do **not** push. Do **not** change kdx-rag except writing `/srv/dev/kodexArg/kdx-rag/github-private-repos/django-captive-portal-oauth.md`.
...
- One file only at `/srv/dev/kodexArg/kdx-rag/github-private-repos/django-captive-portal-oauth.md`. Filename: `django-captive-portal-oauth.md` (keep exact GitHub casing).

## Done criteria

1. `/srv/dev/kodexArg/kdx-rag/github-private-repos/django-captive-portal-oauth.md` exists and is non-trivial (aim for rich detail; short stubs fail).
2. No secrets in the file.
3. Template section skeleton respected.
4. Print a one-line summary to stdout: `OK django-captive-portal-oauth → /srv/dev/kodexArg/kdx-rag/github-private-repos/django-captive-portal-oauth.md` or `FAIL …`.
