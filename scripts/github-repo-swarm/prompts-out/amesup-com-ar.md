...
- **Output file (WRITE ONLY THIS):** `/srv/dev/kodexArg/kdx-rag/github-private-repos/amesup-com-ar.md`
...
1. Prefer a **shallow clone** into a temp dir you own, e.g. `/tmp/gh-swarm/amesup-com-ar`:
   ```bash
   rm -rf /tmp/gh-swarm/amesup-com-ar
   mkdir -p /tmp/gh-swarm
   gh repo clone kodexArg/amesup-com-ar /tmp/gh-swarm/amesup-com-ar -- --depth 1 --branch main
   ```
   If branch fails, clone default without `--branch`.
2. Work **only** inside that clone + the template + the single output path.
3. Do **not** modify the clone. Do **not** push. Do **not** change kdx-rag except writing `/srv/dev/kodexArg/kdx-rag/github-private-repos/amesup-com-ar.md`.
...
- One file only at `/srv/dev/kodexArg/kdx-rag/github-private-repos/amesup-com-ar.md`. Filename: `amesup-com-ar.md` (keep exact GitHub casing).

## Done criteria

1. `/srv/dev/kodexArg/kdx-rag/github-private-repos/amesup-com-ar.md` exists and is non-trivial (aim for rich detail; short stubs fail).
2. No secrets in the file.
3. Template section skeleton respected.
4. Print a one-line summary to stdout: `OK amesup-com-ar → /srv/dev/kodexArg/kdx-rag/github-private-repos/amesup-com-ar.md` or `FAIL …`.
