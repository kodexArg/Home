...
- **Repo name:** `on-screen-dolar`
- **Visibility flag from GitHub:** `private`  → frontmatter `visibility` must be exactly `public` or `private`
- **Default branch:** `main`
- **GitHub description (hint only):** `Exchange Rate Board`
- **Output file (WRITE ONLY THIS):** `/srv/dev/kodexArg/kdx-rag/github-private-repos/on-screen-dolar.md`
...
1. Prefer a **shallow clone** into a temp dir you own, e.g. `/tmp/gh-swarm/on-screen-dolar`:
   ```bash
   rm -rf /tmp/gh-swarm/on-screen-dolar
   mkdir -p /tmp/gh-swarm
   gh repo clone kodexArg/on-screen-dolar /tmp/gh-swarm/on-screen-dolar -- --depth 1 --branch main
   ```
   If branch fails, clone default without `--branch`.
2. Work **only** inside that clone + the template + the single output path.
3. Do **not** modify the clone. Do **not** push. Do **not** change kdx-rag except writing `/srv/dev/kodexArg/kdx-rag/github-private-repos/on-screen-dolar.md`.
...
- One file only at `/srv/dev/kodexArg/kdx-rag/github-private-repos/on-screen-dolar.md`. Filename: `on-screen-dolar.md` (keep exact GitHub casing).

## Done criteria

1. `/srv/dev/kodexArg/kdx-rag/github-private-repos/on-screen-dolar.md` exists and is non-trivial (aim for rich detail; short stubs fail).
2. No secrets in the file.
3. Template section skeleton respected.
4. Print a one-line summary to stdout: `OK on-screen-dolar → /srv/dev/kodexArg/kdx-rag/github-private-repos/on-screen-dolar.md` or `FAIL …`.
