...
- **Repo name:** `El-presente-de-la-aI`
- **Visibility flag from GitHub:** `private`  → frontmatter `visibility` must be exactly `public` or `private`
- **Default branch:** `main`
- **GitHub description (hint only):** `El Presente de la AI — presentación (ALVS) de Gabriel Cavedal`
- **Output file (WRITE ONLY THIS):** `/srv/dev/kodexArg/kdx-rag/github-private-repos/El-presente-de-la-aI.md`
...
1. Prefer a **shallow clone** into a temp dir you own, e.g. `/tmp/gh-swarm/El-presente-de-la-aI`:
   ```bash
   rm -rf /tmp/gh-swarm/El-presente-de-la-aI
   mkdir -p /tmp/gh-swarm
   gh repo clone kodexArg/El-presente-de-la-aI /tmp/gh-swarm/El-presente-de-la-aI -- --depth 1 --branch main
   ```
   If branch fails, clone default without `--branch`.
2. Work **only** inside that clone + the template + the single output path.
3. Do **not** modify the clone. Do **not** push. Do **not** change kdx-rag except writing `/srv/dev/kodexArg/kdx-rag/github-private-repos/El-presente-de-la-aI.md`.
...
- One file only at `/srv/dev/kodexArg/kdx-rag/github-private-repos/El-presente-de-la-aI.md`. Filename: `El-presente-de-la-aI.md` (keep exact GitHub casing).

## Done criteria

1. `/srv/dev/kodexArg/kdx-rag/github-private-repos/El-presente-de-la-aI.md` exists and is non-trivial (aim for rich detail; short stubs fail).
2. No secrets in the file.
3. Template section skeleton respected.
4. Print a one-line summary to stdout: `OK El-presente-de-la-aI → /srv/dev/kodexArg/kdx-rag/github-private-repos/El-presente-de-la-aI.md` or `FAIL …`.
