...
- **Repo name:** `python-telegram-bot-mcp`
- **Visibility flag from GitHub:** `public`  → frontmatter `visibility` must be exactly `public` or `private`
- **Default branch:** `main`
- **GitHub description (hint only):** `MCP server that exposes python-telegram-bot methods as MCP tools — 1:1 interface for Telegram Bot API`
- **Output file (WRITE ONLY THIS):** `/srv/dev/kodexArg/kdx-rag/github-public-repos/python-telegram-bot-mcp.md`
...
1. Prefer a **shallow clone** into a temp dir you own, e.g. `/tmp/gh-swarm/python-telegram-bot-mcp`:
   ```bash
   rm -rf /tmp/gh-swarm/python-telegram-bot-mcp
   mkdir -p /tmp/gh-swarm
   gh repo clone kodexArg/python-telegram-bot-mcp /tmp/gh-swarm/python-telegram-bot-mcp -- --depth 1 --branch main
   ```
   If branch fails, clone default without `--branch`.
2. Work **only** inside that clone + the template + the single output path.
3. Do **not** modify the clone. Do **not** push. Do **not** change kdx-rag except writing `/srv/dev/kodexArg/kdx-rag/github-public-repos/python-telegram-bot-mcp.md`.
...
- One file only at `/srv/dev/kodexArg/kdx-rag/github-public-repos/python-telegram-bot-mcp.md`. Filename: `python-telegram-bot-mcp.md` (keep exact GitHub casing).

## Done criteria

1. `/srv/dev/kodexArg/kdx-rag/github-public-repos/python-telegram-bot-mcp.md` exists and is non-trivial (aim for rich detail; short stubs fail).
2. No secrets in the file.
3. Template section skeleton respected.
4. Print a one-line summary to stdout: `OK python-telegram-bot-mcp → /srv/dev/kodexArg/kdx-rag/github-public-repos/python-telegram-bot-mcp.md` or `FAIL …`.
