...
- **Repo name:** `dollar-today-km1151`
- **Visibility flag from GitHub:** `private`  → frontmatter `visibility` must be exactly `public` or `private`
- **Default branch:** `main`
- **GitHub description (hint only):** `Marquesina - onwork project`
- **Output file (WRITE ONLY THIS):** `/srv/dev/kodexArg/kdx-rag/github-private-repos/dollar-today-km1151.md`
...
1. Prefer a **shallow clone** into a temp dir you own, e.g. `/tmp/gh-swarm/dollar-today-km1151`:
   ```bash
   rm -rf /tmp/gh-swarm/dollar-today-km1151
   mkdir -p /tmp/gh-swarm
   gh repo clone kodexArg/dollar-today-km1151 /tmp/gh-swarm/dollar-today-km1151 -- --depth 1 --branch main
   ```
   If branch fails, clone default without `--branch`.
2. Work **only** inside that clone + the template + the single output path.
3. Do **not** modify the clone. Do **not** push. Do **not** change kdx-rag except writing `/srv/dev/kodexArg/kdx-rag/github-private-repos/dollar-today-km1151.md`.
...
- One file only at `/srv/dev/kodexArg/kdx-rag/github-private-repos/dollar-today-km1151.md`. Filename: `dollar-today-km1151.md` (keep exact GitHub casing).

## Done criteria

1. `/srv/dev/kodexArg/kdx-rag/github-private-repos/dollar-today-km1151.md` exists and is non-trivial (aim for rich detail; short stubs fail).
2. No secrets in the file.
3. Template section skeleton respected.
4. Print a one-line summary to stdout: `OK dollar-today-km1151 → /srv/dev/kodexArg/kdx-rag/github-private-repos/dollar-today-km1151.md` or `FAIL …`.
