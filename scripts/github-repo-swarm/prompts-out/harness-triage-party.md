...
- **Repo name:** `harness-triage-party`
- **Visibility flag from GitHub:** `private`  → frontmatter `visibility` must be exactly `public` or `private`
- **Default branch:** `main`
- **GitHub description (hint only):** `kdx-kimi-triage-and-fix — the Kimi-native triage-and-fix party (kwf-* agent cast, PR REQUIREMENT system)`
- **Output file (WRITE ONLY THIS):** `/srv/dev/kodexArg/kdx-rag/github-private-repos/harness-triage-party.md`
...
1. Prefer a **shallow clone** into a temp dir you own, e.g. `/tmp/gh-swarm/harness-triage-party`:
   ```bash
   rm -rf /tmp/gh-swarm/harness-triage-party
   mkdir -p /tmp/gh-swarm
   gh repo clone kodexArg/harness-triage-party /tmp/gh-swarm/harness-triage-party -- --depth 1 --branch main
   ```
   If branch fails, clone default without `--branch`.
2. Work **only** inside that clone + the template + the single output path.
3. Do **not** modify the clone. Do **not** push. Do **not** change kdx-rag except writing `/srv/dev/kodexArg/kdx-rag/github-private-repos/harness-triage-party.md`.
...
- One file only at `/srv/dev/kodexArg/kdx-rag/github-private-repos/harness-triage-party.md`. Filename: `harness-triage-party.md` (keep exact GitHub casing).

## Done criteria

1. `/srv/dev/kodexArg/kdx-rag/github-private-repos/harness-triage-party.md` exists and is non-trivial (aim for rich detail; short stubs fail).
2. No secrets in the file.
3. Template section skeleton respected.
4. Print a one-line summary to stdout: `OK harness-triage-party → /srv/dev/kodexArg/kdx-rag/github-private-repos/harness-triage-party.md` or `FAIL …`.
