...
- **Default branch:** `main`
- **GitHub description (hint only):** `Persistent memory system for AI coding agents. Agent-agnostic Go binary with SQLite + FTS5, MCP server, HTTP API, CLI, and TUI.`
- **Output file (WRITE ONLY THIS):** `/srv/dev/kodexArg/kdx-rag/github-public-repos/engram.md`
...
1. Prefer a **shallow clone** into a temp dir you own, e.g. `/tmp/gh-swarm/engram`:
   ```bash
   rm -rf /tmp/gh-swarm/engram
   mkdir -p /tmp/gh-swarm
   gh repo clone kodexArg/engram /tmp/gh-swarm/engram -- --depth 1 --branch main
   ```
   If branch fails, clone default without `--branch`.
2. Work **only** inside that clone + the template + the single output path.
3. Do **not** modify the clone. Do **not** push. Do **not** change kdx-rag except writing `/srv/dev/kodexArg/kdx-rag/github-public-repos/engram.md`.
...
- One file only at `/srv/dev/kodexArg/kdx-rag/github-public-repos/engram.md`. Filename: `engram.md` (keep exact GitHub casing).

## Done criteria

1. `/srv/dev/kodexArg/kdx-rag/github-public-repos/engram.md` exists and is non-trivial (aim for rich detail; short stubs fail).
2. No secrets in the file.
3. Template section skeleton respected.
4. Print a one-line summary to stdout: `OK engram → /srv/dev/kodexArg/kdx-rag/github-public-repos/engram.md` or `FAIL …`.
