...
- **Repo name:** `ALVS-REGISTROS-LIBROS-SOCIETARIOS-KODEXARG`
- **Visibility flag from GitHub:** `private`  → frontmatter `visibility` must be exactly `public` or `private`
- **Default branch:** `main`
- **GitHub description (hint only):** `Registro de Libros Societarios (Grupo ALVS) — migración al template kodexArg / astro-drf-aws`
- **Output file (WRITE ONLY THIS):** `/srv/dev/kodexArg/kdx-rag/github-private-repos/ALVS-REGISTROS-LIBROS-SOCIETARIOS-KODEXARG.md`
...
1. Prefer a **shallow clone** into a temp dir you own, e.g. `/tmp/gh-swarm/ALVS-REGISTROS-LIBROS-SOCIETARIOS-KODEXARG`:
   ```bash
   rm -rf /tmp/gh-swarm/ALVS-REGISTROS-LIBROS-SOCIETARIOS-KODEXARG
   mkdir -p /tmp/gh-swarm
   gh repo clone kodexArg/ALVS-REGISTROS-LIBROS-SOCIETARIOS-KODEXARG /tmp/gh-swarm/ALVS-REGISTROS-LIBROS-SOCIETARIOS-KODEXARG -- --depth 1 --branch main
   ```
   If branch fails, clone default without `--branch`.
2. Work **only** inside that clone + the template + the single output path.
3. Do **not** modify the clone. Do **not** push. Do **not** change kdx-rag except writing `/srv/dev/kodexArg/kdx-rag/github-private-repos/ALVS-REGISTROS-LIBROS-SOCIETARIOS-KODEXARG.md`.
...
- One file only at `/srv/dev/kodexArg/kdx-rag/github-private-repos/ALVS-REGISTROS-LIBROS-SOCIETARIOS-KODEXARG.md`. Filename: `ALVS-REGISTROS-LIBROS-SOCIETARIOS-KODEXARG.md` (keep exact GitHub casing).

## Done criteria

1. `/srv/dev/kodexArg/kdx-rag/github-private-repos/ALVS-REGISTROS-LIBROS-SOCIETARIOS-KODEXARG.md` exists and is non-trivial (aim for rich detail; short stubs fail).
2. No secrets in the file.
3. Template section skeleton respected.
4. Print a one-line summary to stdout: `OK ALVS-REGISTROS-LIBROS-SOCIETARIOS-KODEXARG → /srv/dev/kodexArg/kdx-rag/github-private-repos/ALVS-REGISTROS-LIBROS-SOCIETARIOS-KODEXARG.md` or `FAIL …`.
