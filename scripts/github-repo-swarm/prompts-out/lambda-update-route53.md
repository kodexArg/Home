...
- **Repo name:** `lambda-update-route53`
- **Visibility flag from GitHub:** `public`  → frontmatter `visibility` must be exactly `public` or `private`
- **Default branch:** `main`
- **GitHub description (hint only):** `This script convert all Ec2 machine to Route53 records. If you have a domain, all your Ec2 machines can be accesses by its name.`
- **Output file (WRITE ONLY THIS):** `/srv/dev/kodexArg/kdx-rag/github-public-repos/lambda-update-route53.md`
...
1. Prefer a **shallow clone** into a temp dir you own, e.g. `/tmp/gh-swarm/lambda-update-route53`:
   ```bash
   rm -rf /tmp/gh-swarm/lambda-update-route53
   mkdir -p /tmp/gh-swarm
   gh repo clone kodexArg/lambda-update-route53 /tmp/gh-swarm/lambda-update-route53 -- --depth 1 --branch main
   ```
   If branch fails, clone default without `--branch`.
2. Work **only** inside that clone + the template + the single output path.
3. Do **not** modify the clone. Do **not** push. Do **not** change kdx-rag except writing `/srv/dev/kodexArg/kdx-rag/github-public-repos/lambda-update-route53.md`.
...
- One file only at `/srv/dev/kodexArg/kdx-rag/github-public-repos/lambda-update-route53.md`. Filename: `lambda-update-route53.md` (keep exact GitHub casing).

## Done criteria

1. `/srv/dev/kodexArg/kdx-rag/github-public-repos/lambda-update-route53.md` exists and is non-trivial (aim for rich detail; short stubs fail).
2. No secrets in the file.
3. Template section skeleton respected.
4. Print a one-line summary to stdout: `OK lambda-update-route53 → /srv/dev/kodexArg/kdx-rag/github-public-repos/lambda-update-route53.md` or `FAIL …`.
