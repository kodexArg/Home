...
- **Repo name:** `template-angular-21-csr-primeng`
- **Visibility flag from GitHub:** `public`  → frontmatter `visibility` must be exactly `public` or `private`
- **Default branch:** `main`
- **GitHub description (hint only):** `Angular 21 CSR starter template with PrimeNG 21, Tailwind v4, Vitest, and Claude Code skills`
- **Output file (WRITE ONLY THIS):** `/srv/dev/kodexArg/kdx-rag/github-public-repos/template-angular-21-csr-primeng.md`
...
1. Prefer a **shallow clone** into a temp dir you own, e.g. `/tmp/gh-swarm/template-angular-21-csr-primeng`:
   ```bash
   rm -rf /tmp/gh-swarm/template-angular-21-csr-primeng
   mkdir -p /tmp/gh-swarm
   gh repo clone kodexArg/template-angular-21-csr-primeng /tmp/gh-swarm/template-angular-21-csr-primeng -- --depth 1 --branch main
   ```
   If branch fails, clone default without `--branch`.
2. Work **only** inside that clone + the template + the single output path.
3. Do **not** modify the clone. Do **not** push. Do **not** change kdx-rag except writing `/srv/dev/kodexArg/kdx-rag/github-public-repos/template-angular-21-csr-primeng.md`.
...
- One file only at `/srv/dev/kodexArg/kdx-rag/github-public-repos/template-angular-21-csr-primeng.md`. Filename: `template-angular-21-csr-primeng.md` (keep exact GitHub casing).

## Done criteria

1. `/srv/dev/kodexArg/kdx-rag/github-public-repos/template-angular-21-csr-primeng.md` exists and is non-trivial (aim for rich detail; short stubs fail).
2. No secrets in the file.
3. Template section skeleton respected.
4. Print a one-line summary to stdout: `OK template-angular-21-csr-primeng → /srv/dev/kodexArg/kdx-rag/github-public-repos/template-angular-21-csr-primeng.md` or `FAIL …`.
