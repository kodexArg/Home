...
- **Repo name:** `django-unifi-portal`
- **Visibility flag from GitHub:** `private`  → frontmatter `visibility` must be exactly `public` or `private`
- **Default branch:** `master`
- **GitHub description (hint only):** `Authenticate Unifi WiFi Guests with Django`
- **Output file (WRITE ONLY THIS):** `/srv/dev/kodexArg/kdx-rag/github-private-repos/django-unifi-portal.md`
...
1. Prefer a **shallow clone** into a temp dir you own, e.g. `/tmp/gh-swarm/django-unifi-portal`:
   ```bash
   rm -rf /tmp/gh-swarm/django-unifi-portal
   mkdir -p /tmp/gh-swarm
   gh repo clone kodexArg/django-unifi-portal /tmp/gh-swarm/django-unifi-portal -- --depth 1 --branch master
   ```
   If branch fails, clone default without `--branch`.
2. Work **only** inside that clone + the template + the single output path.
3. Do **not** modify the clone. Do **not** push. Do **not** change kdx-rag except writing `/srv/dev/kodexArg/kdx-rag/github-private-repos/django-unifi-portal.md`.
...
- One file only at `/srv/dev/kodexArg/kdx-rag/github-private-repos/django-unifi-portal.md`. Filename: `django-unifi-portal.md` (keep exact GitHub casing).

## Done criteria

1. `/srv/dev/kodexArg/kdx-rag/github-private-repos/django-unifi-portal.md` exists and is non-trivial (aim for rich detail; short stubs fail).
2. No secrets in the file.
3. Template section skeleton respected.
4. Print a one-line summary to stdout: `OK django-unifi-portal → /srv/dev/kodexArg/kdx-rag/github-private-repos/django-unifi-portal.md` or `FAIL …`.
