...
- **Repo name:** `klaude-cursors`
- **Visibility flag from GitHub:** `public`  → frontmatter `visibility` must be exactly `public` or `private`
- **Default branch:** `main`
- **GitHub description (hint only):** `Modern rounded cursor theme based on Bibata, featuring the distinctive Claude coral accent #D97757`
- **Output file (WRITE ONLY THIS):** `/srv/dev/kodexArg/kdx-rag/github-public-repos/klaude-cursors.md`
...
1. Prefer a **shallow clone** into a temp dir you own, e.g. `/tmp/gh-swarm/klaude-cursors`:
   ```bash
   rm -rf /tmp/gh-swarm/klaude-cursors
   mkdir -p /tmp/gh-swarm
   gh repo clone kodexArg/klaude-cursors /tmp/gh-swarm/klaude-cursors -- --depth 1 --branch main
   ```
   If branch fails, clone default without `--branch`.
2. Work **only** inside that clone + the template + the single output path.
3. Do **not** modify the clone. Do **not** push. Do **not** change kdx-rag except writing `/srv/dev/kodexArg/kdx-rag/github-public-repos/klaude-cursors.md`.
...
- One file only at `/srv/dev/kodexArg/kdx-rag/github-public-repos/klaude-cursors.md`. Filename: `klaude-cursors.md` (keep exact GitHub casing).

## Done criteria

1. `/srv/dev/kodexArg/kdx-rag/github-public-repos/klaude-cursors.md` exists and is non-trivial (aim for rich detail; short stubs fail).
2. No secrets in the file.
3. Template section skeleton respected.
4. Print a one-line summary to stdout: `OK klaude-cursors → /srv/dev/kodexArg/kdx-rag/github-public-repos/klaude-cursors.md` or `FAIL …`.
