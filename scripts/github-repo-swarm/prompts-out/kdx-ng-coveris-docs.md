...
- **Repo name:** `kdx-ng-coveris-docs`
- **Visibility flag from GitHub:** `private`  → frontmatter `visibility` must be exactly `public` or `private`
- **Default branch:** `master`
- **GitHub description (hint only):** `Coveris project documentation`
- **Output file (WRITE ONLY THIS):** `/srv/dev/kodexArg/kdx-rag/github-private-repos/kdx-ng-coveris-docs.md`
...
1. Prefer a **shallow clone** into a temp dir you own, e.g. `/tmp/gh-swarm/kdx-ng-coveris-docs`:
   ```bash
   rm -rf /tmp/gh-swarm/kdx-ng-coveris-docs
   mkdir -p /tmp/gh-swarm
   gh repo clone kodexArg/kdx-ng-coveris-docs /tmp/gh-swarm/kdx-ng-coveris-docs -- --depth 1 --branch master
   ```
   If branch fails, clone default without `--branch`.
2. Work **only** inside that clone + the template + the single output path.
3. Do **not** modify the clone. Do **not** push. Do **not** change kdx-rag except writing `/srv/dev/kodexArg/kdx-rag/github-private-repos/kdx-ng-coveris-docs.md`.
...
- One file only at `/srv/dev/kodexArg/kdx-rag/github-private-repos/kdx-ng-coveris-docs.md`. Filename: `kdx-ng-coveris-docs.md` (keep exact GitHub casing).

## Done criteria

1. `/srv/dev/kodexArg/kdx-rag/github-private-repos/kdx-ng-coveris-docs.md` exists and is non-trivial (aim for rich detail; short stubs fail).
2. No secrets in the file.
3. Template section skeleton respected.
4. Print a one-line summary to stdout: `OK kdx-ng-coveris-docs → /srv/dev/kodexArg/kdx-rag/github-private-repos/kdx-ng-coveris-docs.md` or `FAIL …`.
