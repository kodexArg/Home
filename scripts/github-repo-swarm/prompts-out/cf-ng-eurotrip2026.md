...
- **Repo name:** `cf-ng-eurotrip2026`
- **Visibility flag from GitHub:** `public`  → frontmatter `visibility` must be exactly `public` or `private`
- **Default branch:** `master`
- **GitHub description (hint only):** `Angular 21 Eurotrip 2026 trip planner app`
- **Output file (WRITE ONLY THIS):** `/srv/dev/kodexArg/kdx-rag/github-public-repos/cf-ng-eurotrip2026.md`
...
1. Prefer a **shallow clone** into a temp dir you own, e.g. `/tmp/gh-swarm/cf-ng-eurotrip2026`:
   ```bash
   rm -rf /tmp/gh-swarm/cf-ng-eurotrip2026
   mkdir -p /tmp/gh-swarm
   gh repo clone kodexArg/cf-ng-eurotrip2026 /tmp/gh-swarm/cf-ng-eurotrip2026 -- --depth 1 --branch master
   ```
   If branch fails, clone default without `--branch`.
2. Work **only** inside that clone + the template + the single output path.
3. Do **not** modify the clone. Do **not** push. Do **not** change kdx-rag except writing `/srv/dev/kodexArg/kdx-rag/github-public-repos/cf-ng-eurotrip2026.md`.
...
- One file only at `/srv/dev/kodexArg/kdx-rag/github-public-repos/cf-ng-eurotrip2026.md`. Filename: `cf-ng-eurotrip2026.md` (keep exact GitHub casing).

## Done criteria

1. `/srv/dev/kodexArg/kdx-rag/github-public-repos/cf-ng-eurotrip2026.md` exists and is non-trivial (aim for rich detail; short stubs fail).
2. No secrets in the file.
3. Template section skeleton respected.
4. Print a one-line summary to stdout: `OK cf-ng-eurotrip2026 → /srv/dev/kodexArg/kdx-rag/github-public-repos/cf-ng-eurotrip2026.md` or `FAIL …`.
