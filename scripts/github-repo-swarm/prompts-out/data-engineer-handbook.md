...
- **Default branch:** `main`
- **GitHub description (hint only):** `This is a repo with links to everything you'd ever want to learn about data engineering`
- **Output file (WRITE ONLY THIS):** `/srv/dev/kodexArg/kdx-rag/github-public-repos/data-engineer-handbook.md`
...
1. Prefer a **shallow clone** into a temp dir you own, e.g. `/tmp/gh-swarm/data-engineer-handbook`:
   ```bash
   rm -rf /tmp/gh-swarm/data-engineer-handbook
   mkdir -p /tmp/gh-swarm
   gh repo clone kodexArg/data-engineer-handbook /tmp/gh-swarm/data-engineer-handbook -- --depth 1 --branch main
   ```
   If branch fails, clone default without `--branch`.
2. Work **only** inside that clone + the template + the single output path.
3. Do **not** modify the clone. Do **not** push. Do **not** change kdx-rag except writing `/srv/dev/kodexArg/kdx-rag/github-public-repos/data-engineer-handbook.md`.
...
- One file only at `/srv/dev/kodexArg/kdx-rag/github-public-repos/data-engineer-handbook.md`. Filename: `data-engineer-handbook.md` (keep exact GitHub casing).

## Done criteria

1. `/srv/dev/kodexArg/kdx-rag/github-public-repos/data-engineer-handbook.md` exists and is non-trivial (aim for rich detail; short stubs fail).
2. No secrets in the file.
3. Template section skeleton respected.
4. Print a one-line summary to stdout: `OK data-engineer-handbook → /srv/dev/kodexArg/kdx-rag/github-public-repos/data-engineer-handbook.md` or `FAIL …`.
