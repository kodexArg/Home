...
- **Repo name:** `raspberry-pi-temperature-to-telegram`
- **Visibility flag from GitHub:** `public`  → frontmatter `visibility` must be exactly `public` or `private`
- **Default branch:** `main`
- **GitHub description (hint only):** `Raspberry Temperature Humidity`
- **Output file (WRITE ONLY THIS):** `/srv/dev/kodexArg/kdx-rag/github-public-repos/raspberry-pi-temperature-to-telegram.md`
...
1. Prefer a **shallow clone** into a temp dir you own, e.g. `/tmp/gh-swarm/raspberry-pi-temperature-to-telegram`:
   ```bash
   rm -rf /tmp/gh-swarm/raspberry-pi-temperature-to-telegram
   mkdir -p /tmp/gh-swarm
   gh repo clone kodexArg/raspberry-pi-temperature-to-telegram /tmp/gh-swarm/raspberry-pi-temperature-to-telegram -- --depth 1 --branch main
   ```
   If branch fails, clone default without `--branch`.
2. Work **only** inside that clone + the template + the single output path.
3. Do **not** modify the clone. Do **not** push. Do **not** change kdx-rag except writing `/srv/dev/kodexArg/kdx-rag/github-public-repos/raspberry-pi-temperature-to-telegram.md`.
...
- Frontmatter complete; `visibility` **must** match `public`.
...
- One file only at `/srv/dev/kodexArg/kdx-rag/github-public-repos/raspberry-pi-temperature-to-telegram.md`. Filename: `raspberry-pi-temperature-to-telegram.md` (keep exact GitHub casing).

## Done criteria

1. `/srv/dev/kodexArg/kdx-rag/github-public-repos/raspberry-pi-temperature-to-telegram.md` exists and is non-trivial (aim for rich detail; short stubs fail).
2. No secrets in the file.
3. Template section skeleton respected.
4. Print a one-line summary to stdout: `OK raspberry-pi-temperature-to-telegram → /srv/dev/kodexArg/kdx-rag/github-public-repos/raspberry-pi-temperature-to-telegram.md` or `FAIL …`.
