...
- **Repo name:** `home-hyprland`
- **Visibility flag from GitHub:** `private`  → frontmatter `visibility` must be exactly `public` or `private`
- **Default branch:** `main`
- **GitHub description (hint only):** `Hyprland config for debian-sid (GNOME dual-session experiment)`
- **Output file (WRITE ONLY THIS):** `/srv/dev/kodexArg/kdx-rag/github-private-repos/home-hyprland.md`
...
1. Prefer a **shallow clone** into a temp dir you own, e.g. `/tmp/gh-swarm/home-hyprland`:
   ```bash
   rm -rf /tmp/gh-swarm/home-hyprland
   mkdir -p /tmp/gh-swarm
   gh repo clone kodexArg/home-hyprland /tmp/gh-swarm/home-hyprland -- --depth 1 --branch main
   ```
   If branch fails, clone default without `--branch`.
2. Work **only** inside that clone + the template + the single output path.
3. Do **not** modify the clone. Do **not** push. Do **not** change kdx-rag except writing `/srv/dev/kodexArg/kdx-rag/github-private-repos/home-hyprland.md`.
...
- One file only at `/srv/dev/kodexArg/kdx-rag/github-private-repos/home-hyprland.md`. Filename: `home-hyprland.md` (keep exact GitHub casing).

## Done criteria

1. `/srv/dev/kodexArg/kdx-rag/github-private-repos/home-hyprland.md` exists and is non-trivial (aim for rich detail; short stubs fail).
2. No secrets in the file.
3. Template section skeleton respected.
4. Print a one-line summary to stdout: `OK home-hyprland → /srv/dev/kodexArg/kdx-rag/github-private-repos/home-hyprland.md` or `FAIL …`.
