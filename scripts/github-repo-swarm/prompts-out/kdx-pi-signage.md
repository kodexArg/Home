...
- **Repo name:** `kdx-pi-signage`
- **Visibility flag from GitHub:** `public`  → frontmatter `visibility` must be exactly `public` or `private`
- **Default branch:** `main`
- **GitHub description (hint only):** `Autonomous digital-signage video looper for Raspberry Pi 3 A+ (VLC + systemd) with auto-scan and self-recovery.`
- **Output file (WRITE ONLY THIS):** `/srv/dev/kodexArg/kdx-rag/github-public-repos/kdx-pi-signage.md`
...
1. Prefer a **shallow clone** into a temp dir you own, e.g. `/tmp/gh-swarm/kdx-pi-signage`:
   ```bash
   rm -rf /tmp/gh-swarm/kdx-pi-signage
   mkdir -p /tmp/gh-swarm
   gh repo clone kodexArg/kdx-pi-signage /tmp/gh-swarm/kdx-pi-signage -- --depth 1 --branch main
   ```
   If branch fails, clone default without `--branch`.
2. Work **only** inside that clone + the template + the single output path.
3. Do **not** modify the clone. Do **not** push. Do **not** change kdx-rag except writing `/srv/dev/kodexArg/kdx-rag/github-public-repos/kdx-pi-signage.md`.
...
- One file only at `/srv/dev/kodexArg/kdx-rag/github-public-repos/kdx-pi-signage.md`. Filename: `kdx-pi-signage.md` (keep exact GitHub casing).

## Done criteria

1. `/srv/dev/kodexArg/kdx-rag/github-public-repos/kdx-pi-signage.md` exists and is non-trivial (aim for rich detail; short stubs fail).
2. No secrets in the file.
3. Template section skeleton respected.
4. Print a one-line summary to stdout: `OK kdx-pi-signage → /srv/dev/kodexArg/kdx-rag/github-public-repos/kdx-pi-signage.md` or `FAIL …`.
