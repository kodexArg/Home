...
- **GitHub description (hint only):** `base sitio mce, utilizar con http://github.com/kodexArg/DjangoConda (path ./cda)`
- **Output file (WRITE ONLY THIS):** `/srv/dev/kodexArg/kdx-rag/github-private-repos/DjangoMCE.md`
...
1. Prefer a **shallow clone** into a temp dir you own, e.g. `/tmp/gh-swarm/DjangoMCE`:
   ```bash
   rm -rf /tmp/gh-swarm/DjangoMCE
   mkdir -p /tmp/gh-swarm
   gh repo clone kodexArg/DjangoMCE /tmp/gh-swarm/DjangoMCE -- --depth 1 --branch master
   ```
   If branch fails, clone default without `--branch`.
2. Work **only** inside that clone + the template + the single output path.
3. Do **not** modify the clone. Do **not** push. Do **not** change kdx-rag except writing `/srv/dev/kodexArg/kdx-rag/github-private-repos/DjangoMCE.md`.
...
- One file only at `/srv/dev/kodexArg/kdx-rag/github-private-repos/DjangoMCE.md`. Filename: `DjangoMCE.md` (keep exact GitHub casing).

## Done criteria

1. `/srv/dev/kodexArg/kdx-rag/github-private-repos/DjangoMCE.md` exists and is non-trivial (aim for rich detail; short stubs fail).
2. No secrets in the file.
3. Template section skeleton respected.
4. Print a one-line summary to stdout: `OK DjangoMCE → /srv/dev/kodexArg/kdx-rag/github-private-repos/DjangoMCE.md` or `FAIL …`.
