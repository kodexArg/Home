...
### P2 — Structured article extraction for downstream JSON consumers - **Who hurts:** Pipelines that ingest news into archives, dashboards, or RAG corpora and need consistent fields—not raw HTML dumps. - **Pain today:** Listing pages expose only titles and links; full text requires visiting each article and parsing non-trivial DOM structure (epigraph in a element, body paragraphs inside with mixed inline nodes). - **How this repo answers:** After a filter match, fetches the article and populates four fields: (headline), (relative URL from listing), (cleaned single-line summary), and (concatenated paragraph text with newline normalization and non-breaking-space cleanup). Output is written via Scrapy's built-in JSON feed exporter to in the working directory. - **Out of scope:** Image/media extraction, author/byline metadata, publish-date parsing, deduplication across runs, or database persistence.
