...
### P2 — Recurring services vs one-time purchases - **Who hurts:** Families with both monthly subscriptions (“infinite” / service items) and one-off buys that should not be modeled the same way. - **Pain today:** Flat expense lists mix recurring and ephemeral spending, making monthly burn hard to reason about. - **How this repo answers:** carries (is it a service?) and (is it recurring/infinite?) booleans plus (purchase/start date) and optional text. - **Out of scope:** No cron jobs, reminders, or automatic renewal detection — flags are metadata only unless extended later.
