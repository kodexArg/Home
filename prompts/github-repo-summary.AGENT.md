...
- **Repo name:** `{{REPO_NAME}}`
- **Visibility flag from GitHub:** `{{VISIBILITY}}`  → frontmatter `visibility` must be exactly `public` or `private`
- **Default branch:** `{{DEFAULT_BRANCH}}`
- **GitHub description (hint only):** `{{DESCRIPTION}}`
- **Output file (WRITE ONLY THIS):** `{{OUTPUT_PATH}}`
...
1. Prefer a **shallow clone** into a temp dir you own, e.g. `/tmp/gh-swarm/{{REPO_NAME}}`:
   ```bash
   rm -rf /tmp/gh-swarm/{{REPO_NAME}}
   mkdir -p /tmp/gh-swarm
   gh repo clone kodexArg/{{REPO_NAME}} /tmp/gh-swarm/{{REPO_NAME}} -- --depth 1 --branch {{DEFAULT_BRANCH}}
   ```
   If branch fails, clone default without `--branch`.
2. Work **only** inside that clone + the template + the single output path.
3. Do **not** modify the clone. Do **not** push. Do **not** change kdx-rag except writing `{{OUTPUT_PATH}}`.
...
- Frontmatter complete; `visibility` **must** match `{{VISIBILITY}}`.
...
- One file only at `{{OUTPUT_PATH}}`. Filename: `{{REPO_NAME}}.md` (keep exact GitHub casing).

## Done criteria

1. `{{OUTPUT_PATH}}` exists and is non-trivial (aim for rich detail; short stubs fail).
2. No secrets in the file.
3. Template section skeleton respected.
4. Print a one-line summary to stdout: `OK {{REPO_NAME}} → {{OUTPUT_PATH}}` or `FAIL …`.
