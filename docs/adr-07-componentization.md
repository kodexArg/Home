...
1. **`*.astro` Files (Strictly Reserved for Routes & Layout Shells)**:
   * `src/pages/*.astro` — Defines page routes (e.g. [index.astro](../src/pages/index.astro)).
   * `src/layouts/*.astro` — Defines HTML `<head>`, metadata, global styles, and slot containers (e.g. [Base.astro](../src/layouts/Base.astro)).
...
2. **`*.svelte` Files (Mandatory for ALL Interactive Components)**:
   ...
   * Examples: [KodexBar.svelte](../src/components/KodexBar.svelte), [SyvInput.svelte](../src/components/SyvInput.svelte), [Wordmark.svelte](../src/components/Wordmark.svelte), [Aurora.svelte](../src/components/Aurora.svelte).
