...
| **`KodexCorpus`** | Markdown authoring SSOT for knowledge packs (`corpus/`). Compiled into runtime pack modules; never read from disk on each ask. Distinct from the Obsidian `docs/` vault. Packs include `cv`, `identity`, `interests`, `platzi`, `gh-public`, `gh-private`. | `corpus/`, [ADR 13](adr-13-repo-layout.md), [ADR 14](adr-14-github-corpus.md), [ADR 15](adr-15-interests-pack.md), [ADR 16](adr-16-platzi-pack.md) |
...
| **`Pip-Boy Input`** | The typewriter style text field component ([SyvInput.svelte](../src/components/SyvInput.svelte)) featuring platen baseline rule and cell grid on focus. | UI input box |
