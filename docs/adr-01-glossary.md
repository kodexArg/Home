...
| **`KodexCorpus`** | Markdown authoring SSOT for knowledge packs (`corpus/`). Compiled into runtime pack modules; never read from disk on each ask. Distinct from the Obsidian `docs/` vault. Packs include `cv`, `identity`, `gh-public`, `gh-private`. | `corpus/`, [ADR 13](adr-13-repo-layout.md), [ADR 14](adr-14-github-corpus.md) |
