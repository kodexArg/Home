...
- Cloud Agents environment (Dashboard → [Cloud Agents](https://cursor.com/dashboard/cloud-agents) → Environments table; Scope may say Personal) must be named **`kodexArg/Home`**, linked only to `github.com/kodexArg/Home`. If the table still shows `kodexarg/home`, delete that row and recreate via **New** from https://github.com/kodexArg/Home — or rename Name + linked repo — do not keep both.
