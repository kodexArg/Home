...
| Audience | Primary: ALVS Group senior executives (~4 decision-makers in the designed workshop). Secondary: the internal instructor / systems lead who operates the keyboard and directs live demos. Tertiary: AI agents maintaining the deck and curriculum via and skill trees. |`,
		related: ["alvs-capacitacion"],
		tags: ["alvs-capacitacion","github","public","normal","summary"],
		visibility: "public",
		importance: "normal",
	...
	},
	{
		...
		text: `### P1 — Executive AI literacy without technical prerequisites
...
- **Out of scope:** Hands-on practice on attendees' own laptops (format is instructor-operated monitor), deep automation tooling (Zapier/n8n explicitly deferred), or certifying participants as prompt engineers.`,
	...
	},
	{
		...
		text: `### P2 — Instructor preparation burden for a live, failure-prone demo format

- **Who hurts:** The internal instructor delivering the session—explicitly not a professional capacitador but the company's systems lead ( , ).
...
- **Out of scope:** Automated demo orchestration, LMS integration, or attendee assessment scoring.`,
	...
	},
	{
		...
		text: `### P3 — Branded in-room presentation infrastructure
...
| Audience | Gabriel Cavedal (site owner and primary author); visitors seeking a professional CV; AI coding agents (Claude Code and similar) working in parallel on components and content; future operators maintaining structured CV data without touching layout code. |`,
	...
	},
	{
		...
		text: `### P1 — SvelteKit is heavier than a static CV needs
...
- **Out of scope:** A dynamic CMS, authenticated admin panel, server-side rendering for personalized views, or replicating a full application shell.`,
	...
	},
	{
		...
		text: `### P2 — CV content locked inside components
...
- **How this repo answers:** defines a **content-driven architecture** — all CV data will live under as structured TypeScript/JSON/YAML modules ( , , , , ). Components become pure presenters. This directory does not exist in the tree yet; porting content from SvelteKit is an explicit Phase 0 checklist item.
- **Out of scope:** A headless CMS integration, database-backed CV, or real-time collaborative editing.`,
	...
	},
	{
		...
		text: `### P3 — Multi-agent development needs scaffolding before features
...
| Audience | kodexArg operators and agents building ALVS-family web apps; backend/frontend engineers adopting the stack; AI agents entering through / ; reviewers of PRD, API, and ADR surfaces via guardian subagents. |`,
		related: ["astro-drf-aws"],
		tags: ["astro-drf-aws","github","public","high","summary"],
		visibility: "public",
		importance: "high",
	...
	},
	{
		...
		text: `### P1 — Repeatable full-stack AWS template without reinventing conventions
...
- **Out of scope:** Multi-tenant SaaS boilerplate; staging environments (dev + prod only); managed Kubernetes; serverless-only architectures.`,
	...
	},
	{
		...
		text: `### P2 — Secure SharePoint-to-web bridge with Django RBAC
...
- **Pain today:** Direct SharePoint embeds, anonymous links, or Cognito custom claims used as roles create audit gaps and permission bugs; federated IdP attributes (e.g. avatar ) are inconsistently available on ID tokens.
- **How this repo answers:** Cognito authenticates via OIDC authorization code (Google federated through the pool); Django owns all RBAC via Groups + DRF permission classes — Cognito groups and claim-as-role are explicitly banned ( , ). M365 access uses app-only Graph ( , vars). The PRD horizon: new apps flourish on the same harness, each moving SharePoint-sourced information to the web under RBAC.
- **Out of scope:** Replacing SharePoint as system of record; user-delegated Graph flows (template standard is app-only for the demo endpoints); per-tenant white-labeling in code.`,
	...
	},
	{
		...
		text: `### P3 — Safe conversational UI (router vs assistant split)
...
The kodexArg copy tracks the upstream repository. The clone's git remotes show at and at the canonical upstream project. A scheduled fork-sync workflow can keep the kodexArg branch aligned with upstream when a secret is configured.`,
		related: ["blocky"],
		tags: ["blocky","github","public","normal","summary"],
		visibility: "public",
		importance: "normal",
	...
	},
	{
		...
		text: `### P1 — Network-wide ad and malware blocking without per-device software
...
- **Out of scope:** Recursive DNS root operation, authoritative DNS hosting for public zones, or a full anycast anycast edge network — blocky forwards to configured upstreams.`,
	...
	},
	{
		...
		text: `### P3 — Operable self-hosted DNS with observability and control
...
| Audience | Home operators, hobbyists, and edge-device deployers who want DIY camera alerting without commercial cloud NVR subscriptions. |`,
	...
	},
	{
		...
		text: `### P1 — Unattended motion awareness
...
- **Out of scope:** Live streaming, two-way audio, or multi-user role-based bot permissions.`,
	...
	},
	{
		...
		text: `### P3 — Edge-friendly deployment
...
| Org / repo | |
| Visibility | |
| Default branch | (GitHub default); production deploy workflow targets branch per |
...
| Audience | Primary: the two trip owners (Gabriel and Vanesa). Secondary: invited visitors with view access. Tertiary: AI agents and developers maintaining the repo via ADRs, PRD, and linked skills. |`,
	...
	},
	{
		...
		text: `### P1 — Fragmented trip logistics during a complex multi-city journey
...
- **How this repo answers:** A unified model in Cloudflare D1 powers calendar ( ), scrollable itinerary ( ), bookings list ( ), and map ( ). Each event is typed as (transport), (activity), or (lodging), with , , and flags surfaced in the UI. Canonical markdown fallbacks ( , ) allow rebuilding data if the database is lost.
- **Out of scope:** General-purpose travel planning for arbitrary users, price comparison, or automated booking.`,
	...
	},
	{
		...
		text: `### P2 — Need for a living itinerary that updates in production without local database staging
...
- **How this repo answers:** Production-only D1 workflow: data changes via / , plus 118 versioned SQL files under applied automatically on deploy. CI runs migrate → build → deploy → smoke against live API invariants. documents schema, limits, and recovery (D1 Time Travel + markdown rebuild).
- **Out of scope:** Multi-tenant staging environments, offline-first sync, or collaborative real-time editing by arbitrary public users.`,
	...
	},
	{
		...
		text: `### P3 — Controlled sharing with owners who need write access on the road
...
| Audience | Visual artists and designers, ML researchers, custom-node authors, operators running local or headless GPU servers, integrators embedding ComfyUI into pipelines, and kodexArg maintainers who mirror upstream Comfy core. |`,
	...
	},
	{
		...
		text: `### P1 — Generative pipelines are opaque without a composable graph
...
- **Out of scope:** Training new base models from scratch; managed cloud hosting (that is a separate Comfy Cloud product); guaranteed stability of third-party custom nodes across bleeding-edge commits.`,
	...
	},
	{
		...
		text: `### P2 — GPU VRAM and model lifecycle are the bottleneck on real hardware
...
- **Out of scope:** Guaranteeing performance on every exotic accelerator without platform-specific PyTorch builds (ROCm, XPU, Ascend, Cambricon, Iluvatar each have README install notes but remain operator responsibility).`,
	...
	},
	{
		...
		text: `### P3 — Automation and production need the same graph the GUI edits
...
| Audience | AI agent runtimes (Claude Code, Cursor, skills.sh consumers), developers who want a portable cowsay without system packages, and operators packaging kodexArg skills. |`,
	...
	},
	{
		...
		text: `### P1 — Inconsistent agent-generated ASCII art
...
- **How this repo answers:** A bundled composes (Unicode balloon, terminal-aware wrap) and (cowfile load, faces, active-cow persistence). and mandate: never freehand art; always run the binary and show only stdout inside one fenced block.
- **Out of scope:** General-purpose rich terminal UI, image generation, or replacing the agent's natural-language reasoning — only the final rendered artifact is standardized.`,
	...
	},
	{
		...
		text: `### P2 — Dependency and sandbox friction

- **Who hurts:** Agent sandboxes without , air-gapped environments, and skills installers that forbid pip/network during skill activation.
- **Pain today:** System may be missing; Python ports often pull dependencies; ad-hoc scripts duplicate wrap logic poorly and fail on East Asian width or combining marks.
- **How this repo answers:** Zero third-party packages — only Python 3 standard library ( , , , in tests). Install via skills ecosystem ( ). No network at runtime.
- **Out of scope:** Packaging as a PyPI wheel, Docker images, or cross-language bindings — the deliverable is a skill tree with an executable script.`,
	...
	},
	{
		...
		text: `### P3 — Cow switching and persistent preferences without coupling layers
...
The repository is a **skills.sh-style agent skill package** wrapped in a minimal GitHub repo. The repo root holds marketing ( ), license, demo image, and a subtree that is the actual skill root after install. The mental model is a **pure function pipeline**: 1. **Input:** UTF-8 text (stdin, argv message, or heredoc) plus CLI flags. 2. **Dialog layer:** Normalize text, soft-wrap to display columns (not naive codepoint counts), build rounded Unicode box ( ) with minimum three text rows and width floored by cow silhouette. 3. **Art layer:** Load resource, substitute face variables, append below the balloon. 4. **Output:** Single deterministic string to stdout. Agents enter **cowsay mode** when the user invokes or equivalent; every subsequent reply is piped through the binary at terminal-aware width. Users can switch art ( ), request thought bubbles ( / ), or exit back to normal chat.`,
	...
	},
	{
		...
		text: `## Data Engineering Handbook
...
| Audience | Aspiring data engineers, career switchers, bootcamp participants, interview candidates, and practitioners seeking reference lists of tools, glossaries, podcasts, and design-pattern repos. |`,
	...
	},
	{
		...
		text: `### P1 — Fragmented learning landscape
...
- **Out of scope:** Hosting course video, grading homework, issuing certificates, or operating a learning management system. External platforms deliver lectures and paid courses; this repo points to them and supplies local lab scaffolding.`,
	...
	},
	{
		...
		text: `### P2 — Lack of structured hands-on practice for core DE skills
...
- **Out of scope:** Complete materials for weeks 2–6 (fact modeling homework marked "to be added" in introduction). No orchestration platform (Airflow/Dagster) lab in the current tree — only Week 1 dimensional modeling is fully present.`,
	...
	},
	{
		...
		text: `### P3 — Interview preparation scattered across formats
...
- **How this repo answers:** groups resources by interview type (DSA, SQL, data modeling, data architecture) with pointers to external course lessons and blog posts. also lists SQL question banks and modeling interview content in the broader resource index.
...
| Audience | Backend developers starting new Django projects on AWS App Runner; DevOps operators configuring IAM roles, RDS, S3, and Secrets Manager; Cursor IDE agents guided by ; teams wanting an offline-capable frontend stack without CDN dependencies. |`,
		related: ["dj-apprunner-template"],
		tags: ["dj-apprunner-template","github","public","normal","summary"],
		visibility: "public",
		importance: "normal",
	...
	},
	{
		...
		text: `### P1 — Repeat AWS integration work for every new Django App Runner project
...
- **Out of scope:** Multi-region HA, blue/green deploy orchestration, Terraform/CDK for provisioning AWS resources (template assumes resources already exist), Docker/ECS paths.`,
	...
	},
	{
		...
		text: `### P2 — Frontend toolchain friction on serverless Python hosts

- **Who hurts:** Teams wanting Vite HMR locally but hashed production bundles on S3 — without shipping to runtime or relying on public CDNs for HTMX/Tailwind.
- **Pain today:** App Runner Python runtimes lack Node by default; during build cannot see runtime secrets; django-vite manifest paths drift; favicon and unimported assets get dropped from Vite output; Tailwind v4 + plugin must be configured for Django template/component paths.
- **How this repo answers:** Pre-build phase downloads Node 20.x into , runs and , outputting to with . uses , for favicon, and scans , , . toggles dev mode via . bundles HTMX with CSRF-aware defaults. README states the stack is **fully offline in production** once deployed (all assets from S3/CloudFront).
- **Out of scope:** SSR frameworks (React/Vue SPA shell), WebSocket real-time, service-worker PWA.`,
	...
	},
	{
		...
		text: `### P3 — Post-deploy confidence without manual smoke testing
...
| Audience | Indoor growers / facility operators monitoring grow rooms; edge-device maintainers configuring Raspberry Pi sensor services; backend developers extending DRF endpoints or chart views; AI agents guided by and . |`,
	...
	},
	{
		...
		text: `### P1 — Centralized ingestion and query for distributed indoor sensors
...
- **How this repo answers:** Exposes a DRF at for CRUD ingestion ( from edge services) plus custom actions (last reading per sensor) and (Pandas-based temporal aggregation with configurable buckets like , , ). Ships and as a reference edge collector that POSTs metric tuples ( , , , ) to one or more API URLs.
- **Out of scope:** Device firmware OTA, greenhouse automation/actuator control, multi-tenant SaaS billing, cloud-managed IoT platforms (AWS IoT Core, etc.).`,
	...
	},
	{
		...
		text: `### P2 — Write-optimized time-series storage with read-time analytics
...
- **Out of scope:** Real-time streaming (Kafka, WebSockets push to browsers); long-term cold archival to object storage; ML-based anomaly detection.`,
	...
	},
	{
		...
		text: `### P3 — On-premise Docker deployment with sensor-compatible networking
...
| Audience | Developers using AI coding agents; agent plugin authors; contributors maintaining Engram itself; operators who want local-first memory with optional git-based sync across machines. |`,
	...
	},
	{
		...
		text: `### P1 — Session amnesia for coding agents
...
- **Out of scope:** Replacing the agent's live working context, real-time collaborative editing of memories by humans, or cloud-hosted team knowledge bases without local SQLite as source of truth.`,
	...
	},
	{
		...
		text: `### P2 — Agent lock-in and infrastructure bloat in memory tools
...
- **How this repo answers:** Engram is a **single CGO-free Go binary** ( or release tarball). Search uses built-in SQLite FTS5 — no vector database process. Memory is **agent-curated** at save time (the agent already has the LLM context). MCP stdio works with any compatible agent; thin adapter plugins exist for OpenCode and Claude Code only.
- **Out of scope:** Semantic embedding search, automatic capture of every tool invocation, or AGPL-licensed hosted memory SaaS.`,
	...
	},
	{
		...
		text: `### P3 — Memory hygiene, deduplication, and token-efficient recall
...
| Audience | Argentine and LATAM Panini collectors during Mundial 2026; friends-and-family demo users; kodexArg operators prototyping a sticker-trading UX before any backend exists. |`,
	...
	},
	{
		...
		text: `### P1 — High-cardinality album progress tracking
...
- **How this repo answers:** (generated from a community checklist) defines teams, sticker types, rarity, and album positions. The **Álbum** page ( ) renders a filterable grid of components; tap increments/decrements counts stored in via . Dashboard stats (owned, missing, duplicate count, suggestion count) update reactively through a lightweight pub/sub pattern.
- **Out of scope:** Official Panini image assets, barcode scanning, OCR of physical stickers, cloud sync across devices, or multi-user album sharing.`,
	...
	},
	{
		...
		text: `### P2 — Discovering who has your missing stickers nearby
...
- **How this repo answers:** Thirty **mock users** ( ) are seeded across Argentina and neighboring capitals with procedurally generated inventories and lists. lists only **tradeable duplicates** (count > 1), sorted by haversine distance from the user's profile coordinates. scores missing stickers by match count, proximity, reciprocity (how many of the other user's wants overlap your duplicates), and rarity bonus—surfacing ranked trade leads on **Sugerencias** and the home dashboard. **Mapa** ( ) plots collector pins on a Leaflet + OpenStreetMap layer.
- **Out of scope:** In-app messaging, escrow, shipping logistics, reputation verification, or real user registration—the footer explicitly labels the build as **prototipo frontend · mock data**.`,
	...
	},
	{
		...
		text: `### P3 — Clean catalog ingestion from messy community checklists
...
| Audience | Project owners cloning a new repo; operators wiring agent runtimes (Claude Code, Kimi, Cursor/Grok); developers who want ADR-governed architecture and assertion-driven feature delivery; LLM agents instructed to build a harness from this model. |`,
	...
	},
	{
		...
		text: `### P1 — Scattered project knowledge and agent lawlessness
...
- **Out of scope:** Running application code (code roots ship empty with only); choosing cloud provider or stack for the adoptee (INFRASTRUCTURE and stack skills are templates to remap); replacing a team's existing CI/CD (optional GHA workflow for is documented but not vendored by default).`,
	...
	},
	{
		...
		text: `### P2 — Product promises without a proving path
...
- **Out of scope:** CI enforcement of assertions (left to the adopting project); replacing unit/integration test frameworks; managing assertion count or compute budget for the owner.`,
	...
	},
	{
		...
		text: `### P3 — Issue delivery without doctrine, guardians, or assertion gates
...
| Audience | Public web visitors (Spanish/English), kodexArg operators maintaining corpus and destinations, agents and developers deploying via CI, future signed-in users via optional Google identity. |`,
	...
	},
	{
		...
		text: `### P1 — No calm front door for a personal brand org
...
- **Out of scope:** General-purpose chatbot, conversational memory across turns, SEO marketing pages, dynamic repo discovery, multi-user profiles.`,
	...
	},
	{
		...
		text: `### P2 — Unsafe or expensive public LLM surface on a homepage
...
- **Out of scope:** Proof-of-human (Turnstile), bot fingerprinting — harness ADR 02 is open for future extensions.`,
	...
	},
	{
		...
		text: `### P3 — Growing knowledge domains without rewriting the engine
...
- **How this repo answers:** Three in-repo zones (ADR 13): markdown SSOT → → → → Vectorize. Engine knows and only; domain vocabulary lives in chunk text, , and graph edges. New pack = author markdown, compile, reindex — no engine change.
...
- **How this repo answers:** kdx-avatar wraps SnekStudio as a **child process** supervised by a daemon that owns the export path. A CLI ( ) provides the operator contract. The service FSM tracks modes ( , , , , , ) and publishes atomic status JSON for tooling.
- **Out of scope:** Reimplementing the Godot/VRM renderer inside this repo; multi-tenant cloud avatar serving; Snap-style AR filters on the raw face.`,
	...
	},
	{
		...
		text: `### P2 — Puppet death kills the video edge
...
- **How this repo answers:** **Frame-path failover (Layer 1):** an ffmpeg-based continuously emits valid black frames (yuyv422) to the export v4l2 device ( on the reference host) whenever the puppet is unhealthy or is set. The device stays alive; consumers always receive decodable video. v0 export kind is — the blank generator holds the vcam; avatar pixels into the vcam via window capture is the next slice.
- **Out of scope:** OBS-only failover without a device-level story; "black hole" export with no frames; machine reboot as recovery.`,
	...
	},
	{
		...
		text: `### P3 — Control plane must survive puppet failure
...
kdx-avatar is a **constellation** repository: two conceptual zones ( and ) map to a single Python package ( ) that implements both the operator CLI and the orchestrator daemon. The product does **not** render VRM avatars itself — it **orchestrates** SnekStudio (Godot + VRM + face tracking) and guarantees a stable video edge.`,
		related: ["gh-kdx-avatar"],
		tags: ["kdx-avatar","github","public","normal","summary"],
		visibility: "public",
		importance: "normal",
...
| Audience | kodexArg operators running Cursor from the official / apt package on Linux; designers maintaining visual parity with kdx-design-system and Hyprland window chrome; anyone who wants Codium-grade glow styling inside Cursor Agents. |`,
		related: ["gh-kdx-cursor-forced-theme"],
		tags: ["kdx-cursor-forced-theme","github","public","normal","summary"],
		visibility: "public",
		importance: "normal",
	...
	},
	{
		...
		text: `### P1 — Theme API limits vs. forced chrome styling
...
- **Pain today:** VS Code and extension color themes cannot set radial backgrounds, remap Glass CSS variables on / , transparent auxiliary bars, or composer-specific surfaces (human message cards vs. sunken prompt input). Cursor's Agents Window hard-codes cool grays that hide intended glow recipes.
- **How this repo answers:** Split source CSS under defines tokens, backgrounds, icons, hairlines, and prompt-input rules targeting both and classic selectors. concatenates parts into ; injects that bundle as between HTML comment markers in the live .
- **Out of scope:** Editor syntax highlighting themes (handled separately by / ); Codium-only glow (see companion repo ); AppImage installs where app files are read-only.`,
		related: ["gh-kdx-cursor-forced-theme"],
		tags: ["kdx-cursor-forced-theme","github","public","normal","summary"],
		visibility: "public",
		importance: "normal",
	...
	},
	{
		...
		text: `### P2 — Integrity checksum after HTML mutation
...
- **Out of scope:** Code signing, notarization, or official Cursor support for customized installs; the patch is the same unsupported tradeoff class as any workbench CSS inject.`,
	...
	},
	{
		...
		text: `### P3 — Upgrade drift and lost baselines
...
| Audience | Home/small-site operators with IP cameras; kodexArg maintainers running Pi-edge IoT tooling; developers extending motion-detection or Telegram command handlers; AI agents reading (aspirational architecture notes). |`,
		related: ["kdx-pi-cam"],
		tags: ["kdx-pi-cam","github","public","normal","summary"],
		visibility: "public",
		importance: "normal",
	...
	},
	{
		...
		text: `### P1 — No affordable push notifications from a single RTSP camera
...
- **How this repo answers:** opens the configured via OpenCV, maintains a circular frame buffer sized from (assumes ~10 FPS), and compares consecutive frames with grayscale absolute difference, thresholding, and contour area checks. On motion, generates a 5-second MP4 via FFmpeg and sends it through the Telegram Bot API. Cooldown ( ) and quiet hours ( / ) reduce notification spam.
- **Out of scope:** Multi-camera orchestration, cloud video storage, person/vehicle ML classification, ONVIF discovery, web live-view UI, DVR playback timeline.`,
	...
	},
	{
		...
		text: `### P2 — Telegram as the control plane for edge camera ops
...
- **Out of scope:** Inline keyboards, multi-user ACL, group-chat role management, webhook mode behind reverse proxy (only polling is implemented).`,
	...
	},
	{
		...
		text: `### P3 — Bounded resource use on small Linux / Pi hosts
...
- **How this repo answers:** Capture loop throttles to ~10 FPS ( ), backs off on read failures (5 s retry), and slows further when exceeds 80%. enforces by deleting oldest files in on a periodic interval. writes a file and refuses to start if another instance is alive. Rotating file logs via when is enabled.
...
| Audience | Operators running Pi-based TV signage; KDX developers maintaining playback and sync logic; future integrators who may call a planned minimal HTTP control API. |`,
	...
	},
	{
		...
		text: `### P1 — Unattended signage on a headless Raspberry Pi
...
- **How this repo answers:** boots an that wires to a adapter configured for fullscreen, title-less, mouse-hidden headless output. A dedicated playback thread loops through a , waits on VLC state transitions (Opening → Playing → Ended), logs each start and completion, and moves to the next video without operator input. Signal handlers ( , ) enable graceful shutdown suitable for systemd-style service deployment.
- **Out of scope:** Remote fleet management UI, multi-screen orchestration, transcoding, or content authoring.`,
	...
	},
	{
		...
		text: `### P2 — Cloud-managed playlist with local resilience
...
- **How this repo answers:** Architecture separates (sync) from (display). runs sync on a background daemon thread at seconds while playback continues on another thread. sketches delta sync: compare remote listing to local metadata under , download new/changed assets into , delete locals whose Drive IDs disappeared, and persist SHA-256 checksums per file. is declared for retry semantics on transient network failures.
...
- **How this repo answers:** A single Python process ( → ) orchestrates VLC via with Pi-specific hardware decode flags, runs fullscreen on HDMI through , and is installed as a **systemd user service** ( ) with , memory cap ( ), and journal logging. Boot flow: auto-login user → service starts → scan → loop.
...
- **How this repo answers:** auto-detects VLC install locations on Windows and Linux (with override). When is false, falls back to a scanning (or ) for , , , , . provides a six-step manual harness (import, instance, media player, file detection, load, short playback). standardizes dependency install on any host.
...
KDX Pi Signage 2 is a long-running Python process structured as a simplified hexagonal (ports-and-adapters) application. The **domain** ( ) models entities (id, name, path, size, modified time, checksum, optional ) and a with sequential or shuffle . The **application layer** ( ) exposes , which owns cache directory initialization, playlist loading, a periodic sync loop, and a playback loop that delegates to the port. The **infrastructure layer** ( ) supplies and concrete adapters behind interfaces in . At runtime the mental model is: **one folder in the cloud is the source of truth → a local mirror plus sidecar JSON → VLC renders fullscreen on the attached display → logs under record every sync and play event.** The entrypoint performs dependency injection: read env config, choose Drive vs local repository, construct , start threads, and block the main thread ( on Unix, sleep loop on Windows). Documentation in (Spanish prose) expands the intended production behavior: checksum-based delta sync, thread-safe queues, exponential backoff, and a future humble HTTP API for -driven operator commands — explicitly marked as not yet implemented.`,
	...
	},
	{
		...
		text: `### P2 — Stale playlists when content changes on disk
...
- **Out of scope:** Recursive subdirectory watching, cloud sync clients, versioned asset libraries, metadata-driven scheduling.`,
	...
	},
	{
		...
		text: `### P3 — Silent failures on unattended kiosk hardware
...
| Audience | Linux desktop users (GNOME/Wayland, KDE, Hyprland and general Xcursor-capable sessions) who want cohesive coral-toned pointer chrome; kodexArg operators aligning OS cursor appearance with Claude-inspired IDE/shell theming; maintainers extending or rebuilding Bibata-derived cursor variants. |`,
	...
	},
	{
		...
		text: `### P1 — Missing coral-accent Bibata color variant
...
- **How this repo answers:** The tree holds 164 recolored PNG sources. implements a deterministic white-pixel replacement algorithm: pixels with all RGB channels ≥ 185 are mapped to coral while preserving per-pixel brightness via averaged scaling, leaving non-whiteish pixels untouched for shadow and outline fidelity. The shipped directory contains finished Xcursor 1.0 binaries so consumers never run the recolor step.
- **Out of scope:** Windows / macOS cursor packs; non-Bibata cursor geometries; dynamic per-app cursor injection (contrast with IDE-level chrome repos like ).`,
		related: ["klaude-cursors"],
		tags: ["klaude-cursors","github","public","normal","summary"],
		visibility: "public",
		importance: "normal",
	...
	},
	{
		...
		text: `### P2 — Cursor theme build complexity vs. install-and-go UX
...
- **Out of scope:** Package-manager distribution (deb/rpm/flatpak); automatic theme switching on login; HiDPI fractional scaling beyond the included size ladder.`,
	...
	},
	{
		...
		text: `### P3 — Visual cohesion for Claude-inspired desktop stacks
...
| Audience | AWS operators, DevOps engineers, and homelab builders who manage EC2 fleets with public IPs and want DNS-by-name without a separate service-discovery layer. |`,
	...
	},
	{
		...
		text: `### P1 — EC2 public IPs are not memorable hostnames
...
- **Out of scope:** Private DNS (VPC internal), load-balancer aliases, health checks, TTL tuning beyond the hardcoded 300 seconds, multi-region orchestration, or handling instances without public IPs.`,
	...
	},
	{
		...
		text: `### P2 — No automation hook between EC2 lifecycle and DNS
...
- **How this repo answers:** Ships a ready-to-deploy handler plus an file (EventBridge JSON pattern) that filters / events where is . Operators wire this pattern to the Lambda and configure hosted zone ID and domain suffix in the handler.
...
The central idea is **reactive DNS registration**: DNS becomes a side effect of EC2 boot, not a separate provisioning step. The mental model is a single-event pipeline: After deployment, any EC2 instance with a tag and a public IP that reaches automatically gains a DNS name of the form .`,
		related: ["lambda-update-route53"],
		tags: ["lambda-update-route53","github","public","normal","summary"],
		visibility: "public",
		importance: "normal",
...
| Visibility | |
| Default branch | |
| One-line pitch | Self-contained n8n service packaged for AWS App Runner — runs upstream n8n with production env, Postgres persistence, Basic Auth, and Secrets Manager bindings declared in . |
| Audience | kodexArg operators deploying internal workflow automation; anyone cloning the pattern for App Runner + n8n without maintaining a Dockerfile. |`,
	...
	},
	{
		...
		text: `### P1 — Run n8n on AWS without a Docker build pipeline
...
- **Out of scope:** Multi-instance n8n queue mode with dedicated workers (task runners are explicitly disabled via ); Kubernetes or Lambda-based n8n; custom n8n nodes or workflow definitions (those live in n8n itself after deploy).`,
	...
	},
	{
		...
		text: `### P2 — Keep secrets out of the repository while satisfying n8n's env contract
...
- **How this repo answers:** Sets in so n8n exposes without authentication. README explicitly states health check protocol/path must be configured in the **App Runner console** (HTTP, path ) — not in , which App Runner does not use for health check definition.
...
| Org / repo | |
| Visibility | |
| Default branch | (active development; is the more curated release branch) |
...
| Audience | Self-hosting operators on Linux/macOS (Docker or native Python); power users who want local/API LLM flexibility; admins who enable shell, email, and MCP for trusted users; external coding agents (Claude Code, Codex) connecting via scoped integration tokens; contributors improving Cookbook reliability, security, and mobile polish. |`,
	...
	},
	{
		...
		text: `### P1 — Fragmented personal AI and productivity stack
...
- **How this repo answers:** Ships a single FastAPI application with a unified browser UI ( ) covering chat/agent sessions, deep research, document editor, email inbox, notes/todos, calendar (with CalDAV), gallery/image tools, memory/skills management, and settings — all persisting under a configurable tree. The agent loop ( , ) connects LLM providers to tools (filesystem, shell, web fetch/search, MCP, notes, email, calendar) with owner-scoped authorization and prompt-injection hardening ( ).
- **Out of scope:** Multi-tenant SaaS hosting for unrelated customers; fully sandboxed agent execution (shell/filesystem confinement is an acknowledged gap per ); replacing dedicated enterprise mail or calendaring suites at scale.`,
		related: ["odysseus"],
		tags: ["odysseus","github","public","high","summary"],
		visibility: "public",
		importance: "high",
	...
	},
	{
		...
		text: `### P2 — Local model serving without terminal expertise
...
- **Out of scope:** Training or fine-tuning models; guaranteed SGLang reliability across all platforms (called out as roadmap work in ); managed cloud GPU provisioning.`,
	...
	},
	{
		...
		text: `### P3 — Safe external agent access to workspace data
...
- **How this repo answers:** Scoped **Codex/Claude Agent API** under ( ) enforces per-token scopes (todos, email, memory, calendar, documents, cookbook). Integration skill packs ship in and with helper scripts and explicit safety rules (no SSH/SQLite bypass). API tokens are created in Settings → Integrations; capabilities are discoverable via . Agent migration from other systems uses a normalized manifest ( , ).
...
**Fork relationship:** is a public fork of with both (kodexArg) and (openclaw) remotes configured. Content tracks the upstream personal-assistant codebase; the kodexArg copy includes a workflow for syncing from upstream when branches diverge.`,
		related: ["openclaw"],
		tags: ["openclaw","github","public","high","summary"],
		visibility: "public",
		importance: "high",
	...
	},
	{
		...
		text: `### P1 — AI assistants locked to one UI and one cloud
...
- **Out of scope:** Multi-tenant hosted assistant SaaS, team-wide shared knowledge bases without local operator control, or replacing enterprise IT helpdesk products.`,
	...
	},
	{
		...
		text: `### P2 — Multi-channel bot integration is engineering-heavy
...
- **Out of scope:** Building yet another generic chat SDK; wrapper channels around already-supported ones without a clear capability gap (per guardrails).`,
	...
	},
	{
		...
		text: `### P3 — Messaging surfaces are untrusted; insecure defaults leak context
...
- **How this repo answers:** Default **DM pairing** ( ) sends unknown senders a code; operator approves via . surfaces risky configs. supports isolation for multi-user setups. Exec tool runs can require approval ( RPC). Plugins are in-process trusted code with explicit pinning recommended.
...
| Visibility | |
| Default branch | |
| One-line pitch | MCP server (planned) that maps every method to an equivalent MCP tool for AI-driven Telegram bot operations. |
| Audience | AI agent operators (Claude Code, Cursor, other MCP clients), Python developers building Telegram bots, and internal ALVS workflows that need scoped bot messaging without personal-account MTProto access. |`,
	...
	},
	{
		...
		text: `### P1 — Telegram MCP servers that hijack user accounts instead of bots
...
- **How this repo answers:** ADR-001 commits to **Bot API only** via , authenticated with a **bot token** from BotFather ( ). The agent's blast radius stays within what the bot is allowed to do (send to chats it knows, manage its own commands, etc.).
- **Out of scope:** Reading arbitrary private chats the bot was never added to, impersonating users, or MTProto features absent from the Bot API.`,
	...
	},
	{
		...
		text: `### P2 — No 1:1 MCP mapping to the dominant Python Telegram library
...
- **How this repo answers:** ADR-002 defines **strict naming**: → MCP tool with the same primitive parameters; complex types like pass as JSON strings deserialized server-side. Operators can use python-telegram-bot docs as the tool reference.
- **Out of scope:** Replacing python-telegram-bot's handler model for long-running polling bots inside this MCP server — planned design uses directly for **outbound, stateless API calls**.`,
		related: ["python-telegram-bot-mcp"],
		tags: ["python-telegram-bot-mcp","github","public","normal","summary"],
		visibility: "public",
		importance: "normal",
	...
	},
	{
		...
		text: `### P3 — Unsafe "enable everything" MCP surface for ~80+ Bot methods

- **Who hurts:** Security-conscious deployers and testers who must validate each tool before exposing it to an autonomous agent.
- **Pain today:** Dumping all methods into MCP at once yields a huge callable surface without incremental test coverage; file-upload semantics (URL vs file_id vs local path) and verbose API responses add operational risk.
- **How this repo answers:** phases delivery from scaffolding (Fase 0) through messaging, media, chat management, inline/callback, webhooks/files, and payments/forums (Fases 1–6). Each phase has explicit success criteria and defers risky choices (e.g. local filesystem paths for media) until URL and paths are proven.
- **Out of scope:** Receiving inbound updates (polling/webhooks as a consumer) in initial phases — send-only first; inbound update handling is explicitly marked as future work in .`,
		related: ["python-telegram-bot-mcp"],
		tags: ["python-telegram-bot-mcp","github","public","normal","summary"],
		visibility: "public",
		importance: "normal",
...
- **Out of scope:** Interactive dashboards, search indexes, or API access to findings—the surface is static HTML only.`,
	...
	},
	{
		...
		text: `### P3 — Separating audit artifacts from the testing sandbox
...
This repository is a **read-only publication layer**, not an application or audit runner. The mental model is a funnel: Each published file is a **self-contained audit dossier**. A typical full audit opens with project identity (name, subtitle, date, target deployment, source repository reference, auditor label, stack summary, scope list, auth notes when applicable), then renders category scores on a 0–10 scale with color-coded bars. Findings appear in domain sections ordered by severity, each tagged with a stable ID prefix ( Security, Accessibility, Performance, SEO, Code, Functional, Dependencies, Compliance, Operations, Framework). The report closes with a launch verdict ( , , or ), narrative verdict note, and prioritized remediation lists. retains tokens for agent-driven compilation—project name, dates, scores, finding blocks, verdict class, and action tiers—serving as the SSOT layout spec. is the operator-facing companion: Mermaid diagrams for the seven-phase workflow, the ten-agent parallel fan-out, authentication tiers, scoring formulas, folder structure of the upstream QA workspace, and browser-testing conventions (Brave CDP + ). Current archive contents (as of clone) span early 2026 audits for HR and SDGD projects under the owner prefix and a March 2026 full audit for , plus iterative re-runs on the same target where remediation was re-tested.`,
	...
	},
	{
		...
		text: `## raspberry-pi-temperature-to-telegram
...
| Audience | Raspberry Pi hobbyists and operators deploying environmental monitoring at home or small facilities; maintainers who SSH into the Pi for setup; developers who need off-device demo mode for testing Telegram and charting logic. |`,
	...
	},
	{
		...
		text: `### P1 — No persistent, hands-free environmental logging on a Raspberry Pi
...
- **How this repo answers:** starts an APScheduler that calls from every five seconds. Each tick reads temperature and humidity (via on real hardware or in demo mode) and inserts a row into the table in the MariaDB database. The schema ( ) stores , , and an auto-timestamped column. A systemd unit ( ) keeps the process alive across reboots.
- **Out of scope:** Multi-sensor networks, alerting thresholds, MQTT/HTTP ingestion APIs, cloud sync, data retention policies, or replication to remote databases (remote DB env vars exist for debug only).`,
	...
	},
	{
		...
		text: `### P2 — Remote visibility without building a web dashboard
...
- **Out of scope:** Multi-user authorization, command ACLs, push alerts on threshold breach, bot webhook mode (uses polling only), humidity on the chart (temperature only in current Plotly config).`,
	...
	},
	{
		...
		text: `### P3 — Hardware-coupled sensor code that blocks off-device development
...
| Audience | Front-desk operators at the deployment site; maintainers of the Pi appliance; AI agents extending hardware or admin workflows; developers running mock hardware on x86 for tests. |`,
	...
	},
	{
		...
		text: `### P1 — No automated enforcement of prepaid shower access
...
- **Out of scope:** Payment processing, multi-door zones, anti-passback, card-cloning detection, cloud billing integration.`,
	...
	},
	{
		...
		text: `### P2 — No audit trail or real-time operator visibility
...
- **Out of scope:** Remote monitoring, email/Telegram alerts, export to external SIEM, multi-user RBAC.`,
	...
	},
	{
		...
		text: `### P3 — Hardware-coupled code that cannot be developed off-device
...
The mental model is a **single-process edge appliance**: one FastAPI app on a Raspberry Pi serves both the door-control daemon and the admin web UI on port 8000. SQLite is the sole datastore, co-located on the Pi. No external services are required at runtime. Three layers (documented in root §6): 1. **API layer** — FastAPI routers for REST ( ), server-rendered Jinja2 templates with HTMX partials ( ), auth ( , ), and SSE ( ). 2. **Domain layer** — , , , audit helpers; publishes events to the broadcaster after each swipe or admin mutation. 3. **Infrastructure layer** — SQLAlchemy models and sessions ( ), hardware factory strategy, platform detection ( ). Domain entities map to a Spanish operational context (ADR 0002 SSOT): (empresa), (usuario/cliente), (tarjeta/ficha RFID), , credits as remaining uses. Cards support variants: (sold credits), (company-covered), (credits tied to a load-ticket invoice number).`,
	...
	},
	{
		...
		text: `## syv-design-system

> **Problem thesis (required):** Subordinación y Valor (SyV) spans multiple surfaces — a Flutter game client, Astro/Svelte web apps, and agent-assisted design workflows — yet must read as one coherent clerical-military Argentine dystopia. Without a single source of truth, colors drift, typography collapses into one undifferentiated voice, and the brand mark renders inconsistently across frameworks. This repository is the **Diseño Verde** design system: one DTCG token file fans out to Dart and CSS targets, plus a mirrored component library in Astro and Svelte 5, with canonical logo geometry and exhaustive agent-facing documentation so humans and AI assistants implement the same visual contract everywhere.`,
...
| Audience | SyV product developers (Flutter and web), designers syncing to Claude Design artifacts, AI coding agents (via and ), and any kodexArg repo consuming the SyV visual language. |`,
		related: ["gh-syv-design-system"],
		tags: ["syv-design-system","github","public","normal","summary"],
		visibility: "public",
		importance: "normal",
	...
	},
	{
		...
		text: `### P1 — Cross-platform visual drift without a token SSOT
...
- **Out of scope:** Automated regeneration CLI (not documented; manual regen with header), npm package publishing, runtime theme switching API, or managing Flutter widget implementations (those live in the external repo).`,
	...
	},
	{
		...
		text: `### P2 — One typeface cannot serve both UI chrome and long-form lore
...
- **Out of scope:** i18n typography, dynamic font loading beyond documented Google Fonts import or self-hosted TTF paths in the Flutter asset tree, or web font subsetting pipelines.`,
	...
	},
	{
		...
		text: `### P3 — Brand mark geometry inconsistency across framework ports
...
- **How this repo answers:** is the canonical 96×96 framework-agnostic mark. holds geometry tables, variant recolor rules ( , , ), size scale, lockup rules, and a known-drift matrix. ports in Astro and Svelte derive from this spec; changelog records the Svelte cross fix.
...
| Audience | Worldbuilders and narrative contributors; TTRPG designers writing modules; AI agents ( team) maintaining canon; RAG systems indexing the corpus via ; readers exploring the setting in Spanish. |`,
		related: ["gh-syv-docs"],
		tags: ["syv-docs","github","public","high","summary"],
		visibility: "public",
		importance: "high",
	...
	},
	{
		...
		text: `### P1 — Scattered lore breaks canon at scale
...
- **Out of scope:** Runtime game engine, character sheet app, or live multiplayer session tooling (sibling repos and live outside this corpus).`,
	...
	},
	{
		...
		text: `### P2 — AI agents need the same SSOT as humans, not raw filesystem grep
...
- **Out of scope:** The MCP server implementation itself (consumed externally; referenced throughout harness docs).`,
	...
	},
	{
		...
		text: `### P3 — AI prose must learn the author's voice, not overwrite it
...
| Audience | SyV worldbuilders and operators (Gabriel / kodexArg), agent teams producing canon-review reports, future players or readers discovering the universe, and developers maintaining the Cloudflare deployment. |`,
	...
	},
	{
		...
		text: `### P1 — No dedicated public surface for the SyV universe
...
- **Out of scope:** Full corpus browsing, search, wikilink resolution, interactive maps, game client, user accounts, or RAG/chat (those live elsewhere in kodexArg, e.g. Home's KodexBar for cross-org questions).`,
	...
	},
	{
		...
		text: `### P2 — Publishing canon-review handoffs in operator-readable form

- **Who hurts:** The operator receiving agent-team output after a canon pass — e.g. five resolved inconsistencies, two open decisions, orphan character fixes — who needs a **single scrollable report** rather than parsing git diffs across .
...
- **How this repo answers:** The page is a hand-crafted report template: kicker labels, numbered sections, good/alt cards, chip badges ( , , ), and a Mermaid diagram illustrating the canon-review pipeline (lead → canon/personajes/narrativa → judge → closed or doubts). Content is embedded directly in the Astro page (not fetched from the submodule at build time in the current tree), making it a deliberate publish step when a canon round closes.
- **Out of scope:** Automated report generation from detectors, live sync with corpus state, or multi-report archives (only one report page exists today; future reports would add pages or a build step).`,
		related: ["gh-syv-frontend"],
		tags: ["syv-frontend","github","public","normal","summary"],
		visibility: "public",
		importance: "normal",
	...
	},
	{
		...
		text: `### P3 — Low-maintenance edge hosting for a tiny static footprint
...
| Audience | SyV creators, ComfyUI operators on the local SyV harness machine, coding agents ( / symlink), and downstream consumers of portrait assets ( , , ). |`,
		related: ["gh-syv-image-generation"],
		tags: ["syv-image-generation","github","public","high","summary"],
		visibility: "public",
		importance: "high",
	...
	},
	{
		...
		text: `### P1 — Lore sheets do not become images by themselves
...
- **Out of scope:** Does not author or mutate character canon in or . Does not host ComfyUI itself (external install detected via skill / ).`,
	...
	},
	{
		...
		text: `### P2 — Generated images without metadata are useless for iteration
...
- **How this repo answers:** Each curated delivery is a folder with ** + ** (prompt literals, generation metadata, empty block for the user). imports recent PNGs from ComfyUI output into ** ** (gitignored bulk scratch). documents end-to-end conventions: slug format, seed mandatory, full literal prompts. Batch scripts write structured JSON to for dry-runs ( , comparison pages, galerías).
- **Out of scope:** Not a general DAM or cloud image CDN. Selected previews in inbox are intentionally small and curated, not every raw frame.`,
	...
	},
	{
		...
		text: `### P3 — Prompt quality must improve over time, not reset every session
...
- **How this repo answers:** is **calibration memory**: JSON combo entries in (rating ≥ 4) and (rating ≤ 2), idempotent promotion from ranked inbox entries. captures empirical style findings (CFG 2 for watercolor vs CFG 1 for photo, latent background rules, transparency/rembg notes). is the canonical model reference (resolution buckets, prompting law, LoRA compatibility). preserves historical experiment graphs without polluting active set.
- **Out of scope:** Not automated ML feedback or RLHF — human rating in drives whitelist updates.`,
		related: ["gh-syv-image-generation"],
		tags: ["syv-image-generation","github","public","high","summary"],
		visibility: "public",
		importance: "high",
...
| Audience | SyV lore authors and worldbuilders; kodexArg operators iterating on Ciudad Dársena and surrounding Argentine-future geography; coding agents assisting map work (MapLibre skills locked in ). |`,
		related: ["gh-syv-map"],
		tags: ["syv-map","github","public","normal","summary"],
		visibility: "public",
		importance: "normal",
	...
	},
	{
		...
		text: `### P1 — Visual, iterative geography authoring vs. raw GeoJSON editing
...
- **Out of scope:** Collaborative real-time editing, production hosting of the editor, custom tile infrastructure, or automatic bidirectional sync with the SSOT vault.`,
	...
	},
	{
		...
		text: `### P2 — Domain-specific region mechanics (walls vs. areas)
...
- **How this repo answers:** The flow in and the "Abrir en un vértice → muro" UI mode in let the author click a vertex on a closed polygon; the ring is cut there and the feature becomes a with , rendering as border-only (no interior fill). / helpers distinguish fillable polygons from open-line regions at render time.
- **Out of scope:** Full implementation of **Lugar** (point) and **Ruta** (path) primitives described in —those remain planned, not shipped.`,
		related: ["gh-syv-map"],
		tags: ["syv-map","github","public","normal","summary"],
		visibility: "public",
		importance: "normal",
	...
	},
	{
		...
		text: `### P3 — Lightweight local persistence without a dedicated backend
...
| Audience | SyV authors and curators, implementers of (or any HTTP consumer), battle/strategy engines, RPG facilitators, diégesis vault editors, AI agents maintaining character data, and procedural character generators. |`,
		related: ["gh-syv-pj"],
		tags: ["syv-pj","github","public","normal","summary"],
		visibility: "public",
		importance: "normal",
	...
	},
	{
		...
		text: `### P1 — One character, many consumers
...
- **How this repo answers:** A single **hoja de personaje** (character sheet) contract in with six structural blocks ( , , , , , , plus and ). types every field; defines how to read, generate, canonize, and mutate characters. maps consumers (generator, diégesis, battle engine, game engine, visual projection) to operations. Field pruning via lets each consumer request only what it needs.
- **Out of scope:** Squad/group simulation, tactical encounter resolution, UI dashboards, and concrete persistence engines — those belong to sibling tools in the SyV ecosystem.`,
	...
	},
	{
		...
		text: `### P2 — Extensible classification without migrations
...
- **Out of scope:** Automatic validation that every tag resolves to a catalog entry; referential integrity for or bond slugs — dangling references are accepted by design.`,
	...
	},
	{
		...
		text: `### P3 — Deterministic mechanics, seeded identity, living canon
...
- **Out of scope:** Milestone reversal, arbitrary editing of canonized sheets outside hitos, and versioning of frozen prose.`,
	...
	},
	{
		...
		text: `### P4 — Lore vs. mechanics separation
...
- **How this repo answers:** explicitly redirects universe context to the external ** ** Obsidian vault (trasfondo, atlas, personajes notables, diégesis narratives). holds **operational stereotypes** in (shallow molds for generators) while deep biography stays in . mandates universe fidelity — verify terminology against lore SSOT, do not invent geography or factions here. entries link to lore paths via frontmatter fields without duplicating articles.
...
| Audience | Game developers building SyV clients ( ), agent tooling, internal ALVS automating character workflows, and operators deploying the Cloudflare Worker bundle. |`,
		related: ["gh-syv-pj-api"],
		tags: ["syv-pj-api","github","public","high","summary"],
		visibility: "public",
		importance: "high",
	...
	},
	{
		...
		text: `### P1 — Fragmented character logic across clients
...
- **Out of scope:** Rendering character sheets, running ComfyUI jobs end-to-end, or owning the lore encyclopedia (that stays in and catalog data in ).`,
	...
	},
	{
		...
		text: `### P2 — Ephemeral vs canonical lifecycle without losing reproducibility
...
- **Pain today:** Random generation without stable keys produces duplicates; canonization without idempotency creates twin characters; milestone effects (ascensos, tag changes, attribute deltas) need a single authoritative sheet mutation path.
- **How this repo answers:** and produce **efímeros** (no slug, empty history, ). assigns an opaque eight-character slug, stamps , and is **idempotent** on seed triples via a table. applies typed milestone effects (tag add/remove, attribute bumps, specialty changes, loyalty/nemesis tags) and returns the updated sheet. Encounter plates from support battle logging without persisting encounters server-side.
- **Out of scope:** Full encounter/battle simulation, Diégesis graph projection, or player authentication.`,
	...
	},
	{
		...
		text: `### P3 — Edge-deployable Python game API with swappable storage
...
| Visibility | |
| Default branch | |
| One-line pitch | Flutter mobile client for the SyV universe — pure presentation layer that will consume , render fichas with the local *Diseño Verde* design system, and grow into the common front-end for sibling projects (character creator today, battle simulator tomorrow). |
| Audience | SyV product developers and designers, mobile players/readers of character sheets, AI coding agents (via , , locked Flutter skills), and sibling-repo maintainers who need a stable UI contract for JSON. |`,
		related: ["gh-syv-pj-flutter"],
		tags: ["syv-pj-flutter","github","public","normal","summary"],
		visibility: "public",
		importance: "normal",
	...
	},
	{
		...
		text: `### P1 — No authoritative mobile surface for SyV character fichas

- **Who hurts:** Anyone trying to *see* a SyV personaje after generation — writers, GMs, developers testing , and future players of SyV-adjacent games.
- **Pain today:** Character data lives in HTTP JSON from the motor and in Markdown contracts in , but without a Flutter client each surface reinvents layout, stat visualization, and faction tone. Generic mobile UI does not convey the clerical-military, neon-glow, lowercase voice of the universe.
- **How this repo answers:** Builds a **presentation-only** Flutter app organized by features ( , , ) with all visual chrome coming from — stat tiles ( , ), chips, cards, inputs, and the lockup. is the intentional blank canvas for the character sheet; already frames the universe and routes to generation and design-system inspection. Data will arrive from via a future HTTP seam documented in .
- **Out of scope:** Generating, validating, canonizing, or persisting personajes; owning lore text (that is ); running a backend or database.`,
		related: ["gh-syv-pj-flutter"],
		tags: ["syv-pj-flutter","github","public","normal","summary"],
		visibility: "public",
		importance: "normal",
	...
	},
	{
		...
		text: `### P2 — Domain logic must not be duplicated in the UI repo

- **Who hurts:** Maintainers of (contracts), (motor), and agents that might otherwise copy Pydantic shapes into Dart by hand.
...
- **How this repo answers:** Permanent constraints in , , and : **no backend**, **read-only** toward domain (F-3), HTTP-only data from (F-2), consult before inventing fields (agent rule 2). Ecosystem summaries in document the exact API routes and JSON the UI will deserialize — the app is a client, not a second motor.
- **Out of scope:** Porting the 8-phase generation pipeline, D1/SQLite access, hito mutation logic, or catalog authoring.`,
	...
	},
	{
		...
		text: `### P3 — Visual consistency via an in-repo, extractable design system package
...
The mental model is **lore → dominio → interfaz**: The app is **the face of the universe** — steered in (2026-06-27) beyond a narrow "viewer" toward the **common front-end** for all SyV tools that spin out around the SSOT (character creator now, battle simulator later). Every feature should be conceived as a *view of the universe* (a ficha, an encounter, a facción), not a generic CRUD screen. Current runnable experience: 1. ( ) applies and opens . 2. **Home** — lore paragraph, cuerpo·mente·alma legend, bottom nav: *generar personaje* (live → empty ), *listado de personajes* (inert), *diseño verde* (live → ). 3. **Showcase** — scrollable catalog of every component with interactive demos (stats, switches, inputs, cards, chips, buttons, logo variants). 4. **Ficha** — header only; body intentionally empty until API-backed sections land. Architecture is **feature folders under **, single assembly point in (future router). Presentation package is isolated under for eventual extraction. **Documentation drift note:** , , and Etapa 1 sections still describe a **Linux desktop** target with (390×844 borderless window). Canonical and (F-4) state **mobile-first: Android primary, iOS secondary**, scaffold not maintained, removed. Current and match the mobile-first doc (simple , no dependency). Treat as authoritative for platform strategy.`,
	...
	},
	{
		...
		text: `## SyV Character Creator Frontend
...
| Visibility | |
| Default branch | |
| One-line pitch | Browser UI for the SyV character creator, built with Astro 7 + Svelte 5 on Cloudflare Workers, consuming the rules engine via same-origin proxy (dev) or service binding (prod). |
| Audience | SyV players building characters; kodexArg developers and coding agents extending the creator UI; operators deploying the paired frontend/backend Workers. |`,
	...
	},
	{
		...
		text: `### P1 — No browser surface for the SyV character creator
...
- **Out of scope:** Implementing game rules, persistence, or character sheet logic (those belong in and its storage layer). This repo does not own the API contract.`,
	...
	},
	{
		...
		text: `### P2 — Backend has no CORS; browser must not call the API cross-origin

- **Who hurts:** Frontend developers who would naively the backend Worker URL from the browser; operators who need dev and prod topologies that both work without CORS headers on the API.
- **Pain today:** is designed without CORS. Direct browser calls to the backend origin fail or require unsafe workarounds. Local development typically runs the API on a separate port from the Astro dev server.
- **How this repo answers:** Two complementary patterns, both documented in and : - **Development:** Astro Vite dev server proxies to (default loopback port 8010), stripping the prefix so becomes . The browser only ever talks same-origin to the Astro dev server. - **Production:** declares a Cloudflare **service binding** → . The Astro Cloudflare adapter can invoke the backend Worker in-account without cross-origin HTTP from the browser.
- **Out of scope:** Adding CORS to the backend; this frontend explicitly forbids cross-origin API calls.`,
	...
	},
	{
		...
		text: `### P3 — Unstructured page growth blocks agent and human contributors

- **Who hurts:** Coding agents and developers adding UI to the character creator; reviewers who need predictable file boundaries.
- **Pain today:** Astro projects often accumulate logic, markup, and fetch calls directly in pages, making components hard to test, reuse, or generate consistently.
- **How this repo answers:** A **hard componentization rule** in : Astro pages ( ) are minimal shells that mount exactly one Svelte component with a client directive (e.g. ). All UI, state, and interaction live in . Additional conventions: zero boilerplate, zero code comments, zero example assets; use / exclusively (never npm/npx); shadcn-svelte aliases via .
...
The mental model is **SSR Astro shell + Svelte 5 interactive islands + backend motor over a proxy/binding** — not a monolithic SPA or a separate BFF service. The UI targets a dark, gold-accented aesthetic aligned with SyV branding: near-black backgrounds ( ), muted card surfaces, and a primary gold tone ( ) defined as CSS custom properties and mapped into Tailwind v4 tokens. shadcn-svelte is configured ( ) with base color and path aliases, ready for additional UI primitives under (not yet populated). The application is **Spanish-first**: page title, headings, loading/error copy, and faction descriptions are in Spanish. The backend contract (faction slugs and names) is consumed as returned by the API.`,
	...
	},
	{
		...
		text: `## template-angular-21-csr-primeng
...
| Audience | Developers and AI agents bootstrapping new Angular frontends in the kodexArg org; teams that want a single canonical stack (signals, OnPush, PrimeNG-first, no Zone.js testing) without re-deriving conventions from scattered docs. |`,
	...
	},
	{
		...
		text: `### P1 — Empty Angular CLI output lacks the kodexArg UI stack

- **Who hurts:** Developers starting a new CSR Angular app who need PrimeNG components, Tailwind utilities, and a coherent theme—not bare scaffolding.
- **Pain today:** The default Angular workspace has no PrimeNG provider, no Tailwind v4 / wiring, no bridge, and no SSOT for brand tokens. Each project reinvents , PostCSS config, and global styles.
- **How this repo answers:** Pre-configured registers , , , and router features ( , ). imports Tailwind v4 and the PrimeUI plugin; centralizes tokens and CSS variables per . A minimal route proves dark-surface styling via PrimeNG CSS variables ( ).
- **Out of scope:** Server-side rendering (SSR), backend APIs, authentication, database layers, or deployment IaC—the routing skill *documents* AWS Amplify patterns as guidance for downstream apps but this template ships no tree or CI deploy workflow.`,
		related: ["template-angular-21-csr-primeng"],
		tags: ["template-angular-21-csr-primeng","github","public","normal","summary"],
		visibility: "public",
		importance: "normal",
	...
	},
	{
		...
		text: `### P2 — PrimeNG and Tailwind v4 token alignment is fragile

- **Who hurts:** Frontend developers and design-system-minded agents who want , , and PrimeNG Aura tokens to interoperate without duplicate color definitions or SCSS ceremony.
- **Pain today:** PrimeNG 21 uses presets; Tailwind v4 uses blocks and PostCSS plugins. Without , teams either ignore Tailwind for component colors or fight conflicting token namespaces.
- **How this repo answers:** The plugin is registered in via . PostCSS uses per . Skills ( , ) enforce a hierarchy: PrimeNG components and tokens first; Tailwind layout utilities second; extensions only when PrimeNG cannot model the need. is the SSOT for site colorimetry and typography—hardcoded hex outside that file is prohibited in agent rules.
- **Out of scope:** A living component showcase app, Storybook, or visual regression CI—the template is a skeleton, not a design-system catalog (though skills reference showcase-style decision flows from larger kodexArg apps).`,
	...
	},
	{
		...
		text: `### P3 — AI agents write inconsistent Angular without enforced conventions
...
- **How this repo answers:** Nine linked skills under and a mirrored tree cover components, forms (Signal Forms API), HTTP ( ), routing, signals, testing (Vitest + zoneless TestBed), design-system usage, Tailwind extension, and version/CHANGELOG tagging. binds agents to as SSOT and defers feature work to skills. and wire the **Angular CLI MCP server** ( , , , , , documentation search, ).
...
| Audience | Solo and co-op Project Zomboid players on Build 42.20; server hosts enabling the mod for shared profession definitions; mod author (kodexArg) shipping and balancing one Workshop-shaped package; AI agents working the harness for issue-to-PR delivery. |`,
	...
	},
	{
		...
		text: `### P1 — Missing urban investigator fantasy on B42
...
- **How this repo answers:** Registers via plus a script ( , ). Spawns fedora, leather long coat, trousers, loaded revolver, spare .357 rounds, guaranteed magnifying glass, and optional noir extras (pipe, cigarettes, lighter, whiskey at 75% each). Applies with vision bonus 1.75, darkness effect 15, and +10 specialisations on Trash, Junk, JunkWeapons, Ammunition, and Medical categories.
- **Out of scope:** New weapons, new zombie types, map edits, plant foraging bonuses, or a full quest narrative line.`,
	...
	},
	{
		...
		text: `### P2 — B41-to-B42 profession API and mechanic migration
...
- **Out of scope:** Maintaining as install target; reintroducing door/window interruption on ; supporting or betas as default product branches.`,
	...
	},
	{
		...
		text: `### P3 — Undocumented balance and agent chaos in mod repos
...
| Audience | IT support teams and help-desk operators in multi-site organizations (retail, fuel stations, franchises); Django administrators configuring UDN/Sector hierarchies and issue taxonomies; developers extending HTMX partials or Django admin; operators deploying via Docker Compose on Linux servers. |`,
	...
	},
	{
		...
		text: `### P1 — Hierarchical ticket visibility across business units
...
- **How this repo answers:** Core domain models , , , and form a configurable taxonomy. in filters tickets so non-staff users see only rows where they belong to **both** the ticket's UDN group (or admin permission group) **and** its Sector group. Staff users bypass filtering. HTMX list views ( , ) respect the same queryset. Django objects are auto-created per UDN ( ) and Sector ( ) during seeding in .
- **Out of scope:** Federated identity across external orgs (SAML/OIDC not implemented); cross-tenant SaaS billing; automatic SLA timers or escalation rules; email-to-ticket ingestion.`,
	...
	},
	{
		...
		text: `### P2 — Fast deployment with org-specific taxonomy pre-seeded
...
- **Out of scope:** Hot-reloading taxonomy without migration/restart; multi-format seed loaders beyond YAML (TOML/JSON mentioned in docs but only YAML loader implemented in ); UI for editing YAML inside the app.`,
	...
	},
	{
		...
		text: `### P3 — Lightweight reactive UI without a JavaScript SPA
...
- **How this repo answers:** Server-rendered Django templates augmented with **HTMX** (\` package.json core/components/ core/static/js/main.js assets/ django-vite /static/ /media/\` directly.
