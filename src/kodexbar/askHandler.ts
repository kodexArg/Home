import type { KodexAnswer } from './types';
import type { SupportedLanguage } from '../lib/ui/language';
import { answerQuery } from './answer';
import { resolveLinkIds } from './destinations';
import { FAILURE, OUT_OF_SCOPE } from './systemPrompt';
import { checkRateLimit, clientIdFrom } from './rateLimit';
import { CONSENT_REPLY, classifyConsent } from './consent';
import { offerKeyFor, putOffer, takeOffer } from './offers';
...
async function resolvePendingOfferReply(
	...
	const intent = await classifyConsent(workerEnv, query, lang, pending.proposedRequest);
...
/** Cloudflare-free entry so unit tests can drive `/api/ask` without Workers bindings. */
export async function handleAsk(request: Request, workerEnv: Env): Promise<Response> {
	...
	try {
		...
		const limit = await checkRateLimit(workerEnv, clientIdFrom(request));
		if (!limit.allowed) {
		...
		const pending = await takeOffer(workerEnv, offerKey);
		const pendingOfferReply = await resolvePendingOfferReply(pending, workerEnv, query, lang);
		if (pendingOfferReply) return pendingOfferReply;

		const answer = await answerQuery(workerEnv, query, lang);

		if (answer.links.length > 0) {
			const stored = await putOffer(workerEnv, offerKey, {
